...
__export(schema_exports, {
  ...
  activityMergeLog: () => activityMergeLog,
  challengeParticipants: () => challengeParticipants,
  challenges: () => challenges,
  clubMemberships: () => clubMemberships,
  clubs: () => clubs,
  coachingSessionEvents: () => coachingSessionEvents2,
  ...
  garminEpochsAggregate: () => garminEpochsAggregate,
  garminEpochsRaw: () => garminEpochsRaw,
  garminHealthSnapshots: () => garminHealthSnapshots,
  garminMoveIQ: () => garminMoveIQ,
  garminRealtimeData: () => garminRealtimeData,
  garminSkinTemperature: () => garminSkinTemperature,
  garminWebhookEvents: () => garminWebhookEvents,
...
var init_schema = __esm({
  "shared/schema.ts"() {
    ...
    users = pgTable("users", {
      ...
      shortUserId: text("short_user_id").unique(),
      // 8-digit numeric ID for friend sharing (e.g., "12345678")
      ...
      athleticGrade: text("athletic_grade"),
      // "beginner", "intermediate", "advanced", "elite"
      previousRunsCount: integer("previous_runs_count").default(0),
      averageWeeklyMileage: real("average_weekly_mileage").default(0),
      priorRaceExperience: text("prior_race_experience"),
      // "none", "local", "national", "elite"
      injuryHistory: jsonb("injury_history"),
      // [{date, type, resolved}]
      allowAiToneAdaptation: boolean("allow_ai_tone_adaptation").default(true)
      // Let AI adjust tone per session
    });
    ...
    runs = pgTable("runs", {
      ...
      hasGarminData: boolean("has_garmin_data").default(false),
      // TRUE if enriched with Garmin data
      ...
      // meters
      steepestIncline: real("steepest_incline"),
      // percent
      steepestDecline: real("steepest_decline"),
      // percent
      activeCalories: integer("active_calories"),
      // kcal
      restingCalories: integer("resting_calories"),
      // kcal
      estSweatLoss: real("est_sweat_loss"),
      // liters
      // Training plan context (if this run is part of a coached plan)
      linkedWorkoutId: varchar("linked_workout_id"),
      // ID of the planned workout this run executes
      linkedPlanId: varchar("linked_plan_id"),
      // ID of the training plan this run belongs to
      planProgressWeek: integer("plan_progress_week"),
      // Which week of the plan (1-12)
      planProgressWeeks: integer("plan_progress_weeks"),
      // Total weeks in the plan
      workoutType: varchar("workout_type"),
      // "easy", "tempo", "intervals", "long_run", etc.
      workoutIntensity: varchar("workout_intensity"),
      // "z1", "z2", "z3", "z4", "z5"
    ...
    activityMergeLog = pgTable("activity_merge_log", {
      ...
      userId: varchar("user_id").notNull().references(() => users.id),
      mergeScore: real("merge_score").notNull(),
    ...
    notificationPreferences = pgTable("notification_preferences", {
      ...
      coachingPlanReminderTimezone: text("coaching_plan_reminder_timezone").default("UTC")
    ...
    garminWellnessMetrics = pgTable("garmin_wellness_metrics", {
      ...
      pushDistanceInMeters: real("push_distance_meters"),
      // Wheelchair metric
    ...
    garminActivities = pgTable("garmin_activities", {
      ...
      activitySubType: text("activity_sub_type"),
      // MoveIQ sub-type (e.g., Hurdles, Intervals, Trail)
      ...
      summaryId: text("summary_id"),
      // Alternate activity ID from Garmin
      activityDescription: text("activity_description"),
      // Webhook tracking
      userAccessToken: text("user_access_token"),
      // For lookup if needed
    ...
    garminSkinTemperature = pgTable("garmin_skin_temperature", {
      id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
      userId: varchar("user_id").notNull().references(() => users.id),
      date: text("date").notNull(),
      // YYYY-MM-DD
      ...
      // Average skin temp for the day
      minTemperature: real("min_temperature"),
      // Lowest recorded temp
      maxTemperature: real("max_temperature"),
      // Highest recorded temp
      // Context
      temperatureTrendType: text("temperature_trend_type"),
      // STABLE, WARMING, COOLING
      // Time-series data (5-min intervals)
      temperatureReadings: jsonb("temperature_readings"),
      // { "0": 36.5, "300": 36.6, ... }
      // Metadata
      summaryId: text("summary_id"),
      startTimeInSeconds: integer("start_time_in_seconds"),
    ...
    garminBloodPressure = pgTable("garmin_blood_pressure", {
      ...
      // mmHg (bottom number)
      pulse: integer("pulse"),
      // bpm (heart rate at measurement)
      // Measurement Details
      summaryId: text("summary_id"),
      // Garmin summary ID
      sourceType: text("source_type"),
      // 'MANUAL' | 'AUTOMATIC' | 'EXERCISE'
      measurementTimeInSeconds: integer("measurement_time_in_seconds"),
      measurementTimeOffsetInSeconds: integer("measurement_time_offset_in_seconds"),
      // Timezone offset
      // Classification (calculated from systolic/diastolic)
      classification: text("classification"),
      // 'OPTIMAL' | 'NORMAL' | 'ELEVATED' | 'STAGE_1_HYPERTENSION' | 'STAGE_2_HYPERTENSION' | 'CRISIS'
      // Metadata
      rawData: jsonb("raw_data"),
      createdAt: timestamp("created_at").defaultNow()
    });
    garminHealthSnapshots = pgTable("garmin_health_snapshots", {
      id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
      userId: varchar("user_id").notNull().references(() => users.id),
      // Snapshot Metadata
      ...
      spo2Epochs: jsonb("spo2_epochs"),
      // 5-sec interval oxygen
      // Respiration Rate Metrics
      respMinValue: real("resp_min_value"),
      respMaxValue: real("resp_max_value"),
      respAvgValue: real("resp_avg_value"),
      respEpochs: jsonb("resp_epochs"),
      // 5-sec interval respiration
      // Full raw data (for reprocessing if needed)
      rawData: jsonb("raw_data"),
      // Storage Management
      createdAt: timestamp("created_at").defaultNow(),
      expiresAt: timestamp("expires_at")
      // Auto-delete after 30 days for storage optimization
    });
    garminEpochsRaw = pgTable("garmin_epochs_raw", {
      id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
      userId: varchar("user_id").notNull().references(() => users.id),
      // Epoch Date Tracking
      date: text("date").notNull(),
      ...
      activityType: text("activity_type"),
      // WALKING, RUNNING, WHEELCHAIR_PUSHING, SEDENTARY
      intensity: text("intensity"),
      // SEDENTARY | ACTIVE | HIGHLY_ACTIVE
      // Metrics
      met: real("met"),
      // Metabolic equivalent (intensity: 1.0 = resting, 50+ = max exertion)
      ...
      // Wheelchair metric
      distanceInMeters: real("distance_in_meters"),
      pushDistanceInMeters: real("push_distance_in_meters"),
      // Wheelchair metric
      // Metadata
      summaryId: text("summary_id"),
      startTimeOffsetInSeconds: integer("start_time_offset_in_seconds"),
    ...
    garminEpochsAggregate = pgTable("garmin_epochs_aggregate", {
      id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
      userId: varchar("user_id").notNull().references(() => users.id),
      epochDate: text("date").notNull(),
      // YYYY-MM-DD
    ...
    connectedDevices = pgTable("connected_devices", {
      ...
      createdAt: timestamp("created_at").defaultNow(),
      updatedAt: timestamp("updated_at").defaultNow(),
      // NEW: Track permission changes
      grantedScopes: text("granted_scopes")
      // NEW: Comma-separated list of granted OAuth scopes (Garmin only)
    });
    ...
    plannedWorkouts = pgTable("planned_workouts", {
      ...
      hrZoneNumber: integer("hr_zone_number"),
      // 1-5 if this workout focuses on a specific HR zone
      hrZoneMinBpm: integer("hr_zone_min_bpm"),
      // minimum BPM for target zone (calculated from age)
      hrZoneMaxBpm: integer("hr_zone_max_bpm"),
      // maximum BPM for target zone (calculated from age)
      hrZoneScenario: text("hr_zone_scenario"),
      // 'device' | 'history' | 'effort' | null — which scenario applies
      effortDescription: text("effort_description"),
      // For non-device scenarios: "gentle jog", "comfortably hold conversation", etc.
      description: text("description"),
      instructions: text("instructions"),
      // Detailed workout instructions
      // Interval/repeat workout metadata (for workoutType = "intervals" or "hill_repeats")
      intervalCount: integer("interval_count"),
      // Number of repetitions (e.g., 6 for 6x400m)
      intervalDistanceMeters: integer("interval_distance_meters"),
      // Distance of each interval (e.g., 400 for 400m)
      intervalDurationSeconds: integer("interval_duration_seconds"),
      // Duration if time-based (e.g., 120 for 2min)
      restDistanceMeters: integer("rest_distance_meters"),
      // Rest/recovery distance between intervals
      restDurationSeconds: integer("rest_duration_seconds"),
      // Rest/recovery duration between intervals
      intervalTargetPace: text("interval_target_pace"),
      // Target pace for work phase (mm:ss/km)
      restTargetPace: text("rest_target_pace"),
      // Target pace for recovery phase (mm:ss/km)
      intervalHeartRateMin: integer("interval_heart_rate_min"),
      // Min HR for work phase
      intervalHeartRateMax: integer("interval_heart_rate_max"),
      // Max HR for work phase
      restHeartRateMax: integer("rest_heart_rate_max"),
      // Max HR for recovery phase
      ...
      // "build_fitness", "develop_speed", "active_recovery", "endurance"
      sessionIntent: text("session_intent"),
      // What the session is designed to achieve
      createdAt: timestamp("created_at").defaultNow()
    });
    planAdaptations = pgTable("plan_adaptations", {
      id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
      trainingPlanId: varchar("training_plan_id").notNull().references(() => trainingPlans.id),
    ...
    oauthStateStore = pgTable("oauth_state_store", {
      id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
      state: varchar("state").notNull().unique(),
      // The state parameter sent to OAuth provider
      userId: varchar("user_id").notNull(),
      // User who initiated the OAuth flow
      provider: varchar("provider").notNull(),
      // 'garmin', 'strava', etc.
      appRedirect: text("app_redirect"),
      // Deep link URL to redirect back to mobile app
      historyDays: integer("history_days").default(30),
    ...
    webhookFailureQueue = pgTable("webhook_failure_queue", {
      id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
      webhookType: text("webhook_type").notNull(),
      // 'activities', 'sleep', 'daily', etc.
    ...
    garminWebhookEvents = pgTable("garmin_webhook_events", {
      id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
      webhookType: text("webhook_type").notNull().default("activities"),
      // 'activities', 'activity-details', etc.
      ...
      // Garmin device ID
      status: text("status").notNull(),
      // 'received', 'created_run', 'merged_run', 'failed', 'skipped'
      matchScore: real("match_score"),
      // Fuzzy match score (0-100) if merged
      matchedRunId: varchar("matched_run_id"),
      // ID of existing run if merged
      newRunId: varchar("new_run_id"),
      // ID of new run if created
      activityType: text("activity_type"),
      // 'RUNNING', 'WALKING', etc.
      distanceInMeters: real("distance_in_meters"),
      durationInSeconds: integer("duration_in_seconds"),
    ...
    sessionInstructions = pgTable("session_instructions", {
      id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
      plannedWorkoutId: varchar("planned_workout_id").notNull().unique().references(() => plannedWorkouts.id, { onDelete: "cascade" }),
      // Pre-run brief
      ...
      aiDeterminedTone: text("ai_determined_tone").notNull(),
      // "light_fun", "direct", "motivational", "calm", "serious"
      aiDeterminedIntensity: text("ai_determined_intensity").notNull(),
      // "relaxed", "moderate", "intense"
      toneReasoning: text("tone_reasoning"),
      // Why AI chose this tone (for debugging/insights)
      // Coaching style details
      coachingStyle: jsonb("coaching_style").notNull(),
      // { tone, encouragementLevel, detailDepth, technicalDepth }
      insightFilters: jsonb("insight_filters"),
      // { include: [...], exclude: [...] }
    ...
    coachingSessionEvents2 = pgTable("coaching_session_events", {
      id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
      ...
      // "interval_start", "pace_coaching", "recovery_guidance", etc
      eventPhase: text("event_phase"),
      // "warmup", "interval_2_of_6", "recovery", etc
      coachingMessage: text("coaching_message"),
      coachingAudioUrl: text("coaching_audio_url"),
      userMetrics: jsonb("user_metrics"),
      // Current pace, HR, distance, etc at time of coaching
      toneUsed: text("tone_used"),
      // Actual tone delivered
      userEngagement: text("user_engagement"),
      // "positive", "neutral", "struggled"
...
var init_storage = __esm({
  "server/storage.ts"() {
    ...
    DatabaseStorage = class {
      ...
      async getRequestBetweenUsers(requesterId, addresseeId) {
      ...
      async upsertFriendRequest(requesterId, addresseeId, message) {
        ...
        if (existing) {
          const [updated] = await db.update(friendRequests).set({ status: "pending", respondedAt: null, message: message ?? existing.message }).where(eq(friendRequests.id, existing.id)).returning();
          return updated;
        }
        const [request] = await db.insert(friendRequests).values({
          requesterId,
      ...
      async withdrawFriendRequest(id, requesterId) {
        await db.update(friendRequests).set({ status: "withdrawn", respondedAt: /* @__PURE__ */ new Date() }).where(and(eq(friendRequests.id, id), eq(friendRequests.requesterId, requesterId)));
      }
      ...
      async upsertRunAnalysis(runId, analysis) {
        try {
          const existing = await this.getRunAnalysis(runId);
          if (existing && existing.id !== "legacy") {
            const [updated] = await db.update(runAnalyses).set({ analysis }).where(eq(runAnalyses.runId, runId)).returning();
            return updated;
          }
          const [newAnalysis] = await db.insert(runAnalyses).values({ runId, analysis }).returning();
          return newAnalysis;
      ...
      async createConnectedDevice(data) {
        const [device2] = await db.insert(connectedDevices).values(data).returning();
        return device2;
      }
      async updateConnectedDevice(id, data) {
        const [device2] = await db.update(connectedDevices).set(data).where(eq(connectedDevices.id, id)).returning();
        return device2 || void 0;
      }
      ...
      async getOauthState(state) {
        const [result] = await db.select().from(oauthStateStore).where(eq(oauthStateStore.state, state));
      ...
      async claimOauthState(state) {
        const [result] = await db.delete(oauthStateStore).where(eq(oauthStateStore.state, state)).returning();
        return result || void 0;
      }
      ...
      async cleanupExpiredOauthStates() {
        const result = await db.delete(oauthStateStore).where(sql2`${oauthStateStore.expiresAt} < NOW()`).returning({ id: oauthStateStore.id });
        return result.length;
      }
      // Garmin Webhook Events
      async createGarminWebhookEvent(data) {
        const [event] = await db.insert(garminWebhookEvents).values(data).returning();
        return event;
      }
      async updateGarminWebhookEvent(id, data) {
        const [event] = await db.update(garminWebhookEvents).set({ ...data, updatedAt: /* @__PURE__ */ new Date() }).where(eq(garminWebhookEvents.id, id)).returning();
        return event || void 0;
      }
      async getGarminWebhookStats(userId, days) {
        const since = /* @__PURE__ */ new Date();
        since.setDate(since.getDate() - days);
        const events2 = await db.select().from(garminWebhookEvents).where(and(
          eq(garminWebhookEvents.userId, userId),
          sql2`${garminWebhookEvents.createdAt} >= ${since}`
        ));
        const totalReceived = events2.length;
        const totalCreated = events2.filter((e) => e.status === "created_run").length;
        const totalMerged = events2.filter((e) => e.status === "merged_run").length;
        const totalFailed = events2.filter((e) => e.status === "failed").length;
        const totalSkipped = events2.filter((e) => e.status === "skipped").length;
        const scores = events2.filter((e) => e.matchScore !== null).map((e) => e.matchScore || 0);
...
var polly_service_exports = {};
__export(polly_service_exports, {
  buildPollyTTSInstructions: () => buildPollyTTSInstructions,
  getAvailableVoices: () => getAvailableVoices,
  isPollyConfigured: () => isPollyConfigured,
  mapAccentToPollyVoice: () => mapAccentToPollyVoice,
  synthesizeSpeech: () => synthesizeSpeech
});
import { PollyClient, SynthesizeSpeechCommand } from "@aws-sdk/client-polly";
function getRegionForAccent(accent) {
...
function getPollyClient(region) {
...
function mapAccentToPollyVoice(accent, gender) {
  const normalizedAccent = (accent || "british").toLowerCase();
  ...
  const voiceMap = {
    ...
    australian: {
      male: "Stephen",
      // en-AU Neural male (Russell is Standard-only, not Neural)
      female: "Olivia"
      // en-AU Neural female
    },
    irish: {
    ...
    south_african: {
      male: "Ayanda",
      // en-ZA Neural (only one SA Neural voice)
      female: "Ayanda"
      // en-ZA Neural female
    },
    indian: {
      male: "Kajal",
      // en-IN Neural (only one IN Neural voice)
      female: "Kajal"
      // en-IN Neural female
    },
    new_zealand: {
      male: "Aria",
      // en-NZ — no male Neural voice exists, Aria is gender-neutral enough
      female: "Aria"
      // en-NZ Neural female
    }
  };
...
function mapAccentToLanguageCode(accent) {
  const normalizedAccent = (accent || "british").toLowerCase();
  const languageMap = {
    british: "en-GB",
    american: "en-US",
...
async function synthesizeSpeech(text2, accent, gender, tone) {
  ...
  try {
    const command = new SynthesizeSpeechCommand({
      ...
      VoiceId: voiceId,
      Engine: "neural",
      // Use Neural engine for high quality
...
function buildPollyTTSInstructions(tone) {
  if (!tone) return "";
  const instructions = {
...
function getAvailableVoices(accent) {
  const voiceMap = {
...
__export(ai_service_exports, {
  ...
  generateEmotionalCoaching: () => generateEmotionalCoaching,
  generateHeartRateCoaching: () => generateHeartRateCoaching,
  generateIntervalCoaching: () => generateIntervalCoaching,
...
function buildRunHistoryContext(history, currentPace) {
...
Examples of good output: "Quick right turn onto May Street, looking good!", "Left here onto Dublin Road, keep that rhythm!"`;
  ...
  if (triggerType === "pace_coaching" || triggerType === "pace_abandon") {
    const paceDeviationPercent = params.paceDeviationPercent ?? 0;
    const rollingPaceDeviationPercent = params.rollingPaceDeviationPercent ?? 0;
    const projectedFinishSeconds = params.projectedFinishSeconds ?? 0;
    const currentAvgPaceSecondsPerKm = params.currentAvgPaceSecondsPerKm ?? 0;
    const rollingPaceSecondsPerKm = params.rollingPaceSecondsPerKm ?? 0;
    const progressPercent = params.progressPercent ?? 0;
...
Reinforce the good pacing with positive encouragement. Current: ${avgPaceFormatted}/km, target: ${targetPaceFormatted}/km. Tell them they're nailing it!`;
    ...
    let gradientContext = "";
    if (currentGrade && Math.abs(currentGrade) > 2) {
      gradientContext = currentGrade > 0 ? `Note: Runner is currently climbing (${currentGrade.toFixed(1)}% gradient) \u2014 slower pace is expected on hills.` : `Note: Runner is currently descending (${Math.abs(currentGrade).toFixed(1)}% gradient) \u2014 pace naturally speeds up downhill.`;
    }
    let trendContext = "";
...
function getHeartRateZone(hr, age) {
...
async function generateIntervalCoaching(params) {
  const {
    intervalNumber,
    isWorkPhase,
    distanceInPhaseKm,
  ...
  const phaseEmphasis = isWorkPhase ? "This is your work interval \u2014 focus on the target pace and effort zone." : "This is your recovery interval \u2014 bring your heart rate down and get ready for the next effort.";
  let hrContext = "";
...
Give 1\u20132 punchy, direct sentences. ${isWorkPhase ? "Push them hard but safely." : "Help them recover and prepare for the next effort."}`;
  ...
  try {
    ...
    return isWorkPhase ? `You're on rep ${intervalNumber} \u2014 push steady!` : `You're in recovery \u2014 bring your HR down.`;
  }
}
...
async function getElevationCoaching(params) {
...
async function generateEmotionalCoaching(params) {
  const {
    category,
    distance,
    targetDistance,
...
async function generateTTS(text2, voice = "alloy", instructions, coachAccent, coachGender) {
  const { isPollyConfigured: isPollyConfigured2, synthesizeSpeech: synthesizeSpeech2, mapAccentToPollyVoice: mapAccentToPollyVoice2 } = await Promise.resolve().then(() => (init_polly_service(), polly_service_exports));
  if (isPollyConfigured2()) {
    try {
      const pollyVoice = mapAccentToPollyVoice2(coachAccent, coachGender);
      const awsRegion = process.env.AWS_REGION || "ap-southeast-2";
      console.log(`[TTS] Using Polly Neural \u2014 accent: ${coachAccent}, gender: ${coachGender}, voice: ${pollyVoice}, region: ${awsRegion}`);
      const buffer2 = await synthesizeSpeech2(
        text2,
        coachAccent,
        coachGender,
        instructions
      ...
      console.warn(`[TTS] Polly synthesis failed, falling back to OpenAI. accent: ${coachAccent}, voice: ${mapAccentToPollyVoice2(coachAccent, coachGender)}, region: ${process.env.AWS_REGION || "ap-southeast-2"}, error:`, pollyError.message);
    }
  } else {
...
function buildTTSInstructions(coachAccent, coachTone, coachGender, coachName) {
  const parts = [];
...
CURRENT PHASE: ${currentPhase.toUpperCase()}`;
  ...
  if (context.heartRate) {
    const maxHR = calcMaxHR(context.runnerAge);
...
async function generateWellnessAwarePreRunBriefing(params) {
  ...
  if (hasRoute === true) {
    ...
    if (gain < 20 && loss < 20) {
      ...
      terrainDescription = gain > 200 ? "Mountainous/very hilly terrain \u2014 expect significant climbs and descents" : "Hilly route with noticeable climbs";
    } else if (gain > 50 || loss > 50) {
    ...
    let gradientNote = "";
    if (maxGrad > 8) {
      gradientNote = `Steepest section: ${maxGrad.toFixed(1)}\xB0 \u2014 a tough climb, consider walking if needed`;
    } else if (maxGrad > 5) {
      gradientNote = `Steepest section: ${maxGrad.toFixed(1)}\xB0 \u2014 noticeable hill, shorten your stride and maintain effort`;
    } else if (maxGrad > 2) {
      gradientNote = `Steepest section: ${maxGrad.toFixed(1)}\xB0 \u2014 gentle incline`;
    }
    routeInfo = `
ROUTE:
- Distance: ${formatDistanceForTTS(distance)}
...
5. "${hasRoute === true ? "routeInsight" : "readinessInsight"}": ONE sentence (\u226412 words). ${wellnessContext ? "Key readiness or terrain challenge affecting this specific workout." : "One specific motivational detail tied to this workout."}` : `1. "briefing": 2-3 SENTENCES (max 40 words) for a free-form run. Lead with distance${targetPace ? " and target pace" : ""}, mention weather and how it might affect you. ${wellnessContext ? "Include your readiness status." : ""} Be conversational. ${PACE_FORMAT_RULE}
...
Respond as JSON with fields: briefing, intensityAdvice, weatherAdvice, warnings (array), ${hasRoute === true ? "routeInsight" : "readinessInsight"}`;
  try {
    ...
    return {
      ...
      weatherAdvice: parsed.weatherAdvice || void 0,
      warnings: parsed.warnings || [],
      readinessInsight: parsed.readinessInsight || (hasRoute ? void 0 : "Your body is ready for this run."),
      routeInsight: parsed.routeInsight || (hasRoute ? terrainDescription : void 0),
    ...
    return {
      ...
      weatherAdvice: void 0,
      warnings: [],
      readinessInsight: hasRoute ? void 0 : "Listen to your body and adjust intensity as needed.",
      routeInsight: hasRoute ? terrainDescription : void 0,
      weatherAdvantage: weatherAdvantage || void 0
    };
  }
}
...
`;
      coachingEvents.forEach((event, index2) => {
        prompt += `${index2 + 1}. [${event.eventPhase || "general"}] "${event.coachingMessage}" (Tone: ${event.toneUsed}, Engagement: ${event.userEngagement || "not logged"})
`;
      });
    }
    prompt += `
...
`;
    strugglePoints.forEach((sp, i) => {
      const distKm = sp.distanceMeters != null ? (sp.distanceMeters / 1e3).toFixed(2) : "?";
...
async function generateEliteCoaching(params) {
  const {
    ...
    paceTrendDeltaPerKm,
    projectedFinishTime,
    consecutiveConsistentSplits,
    isNegativeSplitting,
    fastestSplitKm,
    fastestSplitPace,
    targetTimeCategory,
    etaOverTargetPercent,
    remainingMeters,
    trainingPlanId,
    workoutType,
    workoutDescription,
...
${progress < 30 ? `- EARLY RUN: Focus on establishing good form \u2014 relaxed shoulders, arms at 90 degrees, slight forward lean from ankles, landing under hips.` : progress < 70 ? `- MID RUN: Focus on efficiency \u2014 are they bouncing too much? Arms crossing midline? Tension creeping into shoulders or jaw? Quick feet.` : `- LATE RUN: Focus on fatigue management \u2014 when tired, form breaks down. Cue them to check posture (tall spine), relax hands (no clenching), drive arms forward.`}
...
Reference at least one data point. Keep it conversational \u2014 this is spoken aloud while running.`;
      systemExtra = isAerobicZone ? "For this Zone 2 session, emphasize comfort and sustainability. Coach breathing rhythm, relaxation, and how to stay comfortable at effort." : "You specialize in running biomechanics and form coaching. Give one specific, actionable technique cue \u2014 not a generic reminder.";
      break;
    }
    case "milestone": {
...
${milestonePercent && milestonePercent <= 25 ? '   - Quarter way: "Great start, settle into your rhythm, lots of running ahead"' : milestonePercent && milestonePercent <= 50 ? `   - Halfway: "You're at the turnaround point \u2014 everything from here is the home stretch"` : '   - Three quarters: "The hard work is almost done \u2014 finish strong"'}
...
Make them feel like they've accomplished something meaningful. Reference their actual numbers.`;
      systemExtra = isAerobicMilestone ? "For Zone 2 milestones, celebrate the aerobic adaptation work happening right now. Reinforce that consistent, steady effort at aerobic pace is exactly how elite runners build their foundation." : "Celebrate the milestone with genuine enthusiasm while weaving in their real data. Make them feel proud of what they've achieved so far.";
      break;
    }
...
${diff > 1 ? `STATUS: ${Math.abs(diff)} minute(s) BEHIND target. They need to pick up the pace gradually \u2014 not panic.` : diff < -1 ? `STATUS: ${Math.abs(diff)} minute(s) AHEAD of target. They have a cushion \u2014 smart pacing.` : `STATUS: ON TARGET. They're executing their race plan perfectly.`}
...
TREND DETECTED: ${paceTrendDirection === "slowing" ? `Pace is GRADUALLY DRIFTING SLOWER \u2014 approximately ${paceTrendDeltaPerKm ? Math.round(paceTrendDeltaPerKm) + "s/km" : "noticeably"} per kilometer. This is different from a sudden struggle \u2014 it's a gradual fade.` : paceTrendDirection === "speeding_up" ? `Pace is GRADUALLY GETTING FASTER \u2014 approximately ${paceTrendDeltaPerKm ? Math.round(paceTrendDeltaPerKm) + "s/km" : "noticeably"} per kilometer. They're building momentum.` : "Pace has been remarkably CONSISTENT across splits."}
...
- Remind them of their target or what good pacing looks like` : paceTrendDirection === "speeding_up" ? `- Reinforce the positive trend \u2014 they're running smart
- Caution against going too fast too early if they're under 60% done
- If they're past 60%, encourage the push` : `- Praise the consistency \u2014 this is disciplined running
...
Reference their actual split data.`;
    ...
    case "elevation_insight": {
      ...
      if (kmSplits && kmSplits.length >= 2) {
        ...
        terrainAnalysis += kmSplits.map((s, i) => {
          let delta = "";
...
- If cadence dropped: "shorten your stride and quicken your feet \u2014 shorter faster steps are more efficient uphill"` : isDownhill ? `DOWNHILL \u2014 They're on a ${Math.abs(currentGrade).toFixed(1)}% descent right now.
YOUR COACHING MUST:
...
- If they're banking time: "great section to recover heart rate while keeping pace up"` : `FLAT/UNDULATING TERRAIN \u2014 The route is ${totalElevationGain && distance > 0.5 && totalElevationGain / distance < 5 ? "very flat with minimal undulation" : "gently undulating"}.
YOUR COACHING MUST:
...
${etaOverTargetPercent !== void 0 && etaOverTargetPercent <= 0 ? `They are ON TRACK or UNDER their target \u2014 tell them! "You're going to beat your target!" or "Your ${tgtMin}-minute goal is RIGHT THERE!"` : `They are within ${etaOverTargetPercent?.toFixed(1)}% of their target \u2014 they can still make it with a push! Tell them exactly what they're chasing.`}`;
...
__export(route_generation_exports, {
  ...
  calculateAngularSpread: () => calculateAngularSpread2,
...
function calculateAngularSpread2(encodedPolyline, startLat, startLng) {
  const points = decodePolyline(encodedPolyline);
...
function isGenuineCircuit(encodedPolyline, startLat, startLng) {
  ...
  const angularSpread = calculateAngularSpread2(encodedPolyline, startLat, startLng);
...
async function fetchElevationForRoute(encodedPolyline) {
  ...
  try {
    ...
    const path3 = samplePoints.map((p) => `${p.lat},${p.lng}`).join("|");
    const url = `https://maps.googleapis.com/maps/api/elevation/json?path=${encodeURIComponent(path3)}&samples=${samplePoints.length}&key=${GOOGLE_MAPS_API_KEY2}`;
    const response = await fetch(url);
    const data = await response.json();
      ...
      if (googleRoute && googleRoute.success) {
        ...
        if (distanceError < 0.4 && backtrackRatio < 0.5 && loopQuality > 0.5) {
          ...
          enhancedRoutes.push({
            ...
            circuitQuality: {
              backtrackRatio,
              angularSpread: calculateAngularSpread3(googleRoute.polyline, startLat, startLng),
              loopQuality
            },
...
async function fetchElevation(encodedPolyline) {
  ...
  try {
    ...
    const samplePoints = points.filter((_, i) => i % Math.max(1, Math.floor(points.length / 50)) === 0);
    const path3 = samplePoints.map((p) => `${p.lat},${p.lng}`).join("|");
    const url = `https://maps.googleapis.com/maps/api/elevation/json?path=${encodeURIComponent(path3)}&samples=${samplePoints.length}&key=${GOOGLE_MAPS_API_KEY3}`;
    const response = await fetch(url);
    const data = await response.json();
    if (data.status !== "OK" || !data.results) {
      return { gain: 0, loss: 0, maxGradientPercent: 0, maxGradientDegrees: 0 };
...
function calculateAngularSpread3(polylineStr, startLat, startLng) {
  const points = decodePolyline2(polylineStr);
  if (points.length < 5) return 0;
  const bearings = [];
  for (const point of points) {
...
__export(garmin_service_exports, {
  ...
  requestGarminBackfill: () => requestGarminBackfill,
...
async function requestGarminBackfill(accessToken, startTime, endTime) {
  ...
  console.log(`\u{1F4E4} Requesting Garmin backfill for ${startDate} \u2192 ${endDate}`);
...
var init_garmin_service = __esm({
  "server/garmin-service.ts"() {
    ...
    garmin_service_default = {
      ...
      getGarminActivities,
      // deprecated — kept for compatibility, returns []
...
var webhook_processor_exports = {};
__export(webhook_processor_exports, {
  default: () => webhook_processor_default,
  getWebhookQueueStats: () => getWebhookQueueStats,
...
async function retryActivityWebhook(webhook) {
...
async function retrySleepWebhook(webhook) {
  console.log("[Webhook Queue] Retrying sleep webhook");
}
async function retryDailySummaryWebhook(webhook) {
  console.log("[Webhook Queue] Retrying daily summary webhook");
}
async function retryHeartRateWebhook(webhook) {
  console.log("[Webhook Queue] Retrying heart rate webhook");
}
async function retryStressWebhook(webhook) {
...
async function getWebhookQueueStats() {
  try {
    ...
    for (const row of byTypeResult.rows) {
      byType[row.webhook_type] = parseInt(row.count, 10);
    }
...
var init_webhook_processor = __esm({
  "server/webhook-processor.ts"() {
    ...
    RETRY_DELAY_MS = 10 * 60 * 1e3;
    webhook_processor_default = {
...
function paceColor(seconds, fastest, slowest) {
...
function buildGpsRouteElite(x, y, w, h, track, paceData) {
...
function buildRouteMapSvg(w, h, run, userName, hasMapTile) {
  const isVertical = h > w;
  const contentEndY = h - LOGO_ZONE_H;
...
function getRouteMapHeight(w, h) {
  const isVertical = h > w;
  const contentEndY = h - LOGO_ZONE_H;
...
function buildNoDataChart(px, py, w, h, s, color, label) {
...
function buildStickerSvg(sticker, run, canvasW, canvasH) {
  ...
  const maxStickerH = Math.round(150 * s);
  ...
  switch (sticker.widgetId) {
    ...
    case "chart-elevation": {
      ...
      const rawGps = run.gpsTrack;
      const gpsElevData = rawGps?.length >= 2 ? rawGps.map((p) => p.elevation ?? p.alt ?? p.altitude ?? null).filter((v) => v !== null) : [];
    ...
    case "text-custom": {
      const text2 = sticker.customText || run.name || "My Run";
      ...
      return `
        ...
        <text x="${px + padX}" y="${py + padY + tFontSize * 0.75}" font-family="${FONT}" font-size="${tFontSize}" font-weight="700" fill="${C.textDark}">${esc(text2)}</text>
      `;
    }
...
async function generateShareImage(req) {
  ...
  if (hasCustomBg) {
    ...
    if (bgRectIdx >= 0) {
      ...
      if (afterFill >= 0) {
        svgContent = svgContent.substring(0, bgRectIdx) + `<rect width="${w}" height="${h}" fill="#000000" opacity="0"/>` + svgContent.substring(afterFill + 3);
      }
    }
  }
  ...
  if (hasCustomBg) {
    try {
      ...
      if (opacity < 1) {
        ...
        bgBuffer = await sharp(bgBuffer).composite([{ input: fadeBuffer, top: 0, left: 0 }]).png().toBuffer();
      }
      svgBuffer = await sharp(bgBuffer).composite([{ input: svgBuffer, top: 0, left: 0 }]).png({ quality: 95 }).toBuffer();
    } catch (e) {
  ...
  if (req.customStickers && req.customStickers.length > 0) {
    ...
    for (const cs of req.customStickers) {
      try {
        ...
        const resized = await sharp(rawBuf).resize(stickerW, stickerH, { fit: "contain", background: { r: 0, g: 0, b: 0, alpha: 0 } }).ensureAlpha().png().toBuffer();
...
function encodePolyline2(points) {
  let encoded = "";
  let prevLat = 0;
  let prevLng = 0;
...
async function fetchMapTileWithRoute(gpsTrack, tileW, tileH, paceData) {
  ...
  const encodedPath = encodePolyline2(sampled);
...
var health_insights_service_exports = {};
__export(health_insights_service_exports, {
  getDailyWellness: () => getDailyWellness,
  getHealthInsights: () => getHealthInsights,
...
async function getRecoveryStatus(userId) {
  ...
  if (metric.bodyBatteryLevel !== null && metric.bodyBatteryLevel !== void 0) {
...
async function getDailyWellness(userId, date) {
  ...
  return {
    ...
    stepsGoal: metric.stepsGoal || 7e3,
...
async function getTrendData(userId, days = 7) {
  const trendData = {
    dates: [],
    sleepDuration: [],
    avgStress: [],
  ...
  metricsData.reverse().forEach((metric) => {
    ...
    if (metric.avgStressLevel !== null && metric.avgStressLevel !== void 0) {
...
function getEmptySnapshot(date) {
  return {
    date,
    avgHeartRate: 0,
    sleepScore: 0,
...
function getBatteryRecommendation(status) {
  switch (status) {
    case "HIGH":
...
function categorizeVO2Max(vo2Max) {
  if (vo2Max > 40) return "EXCELLENT";
  if (vo2Max > 30) return "GOOD";
  if (vo2Max > 20) return "AVERAGE";
  return "BELOW_AVERAGE";
}
function categorizeFitnessAge(age) {
  const chronological = 30;
...
async function fuzzyMatchGarminToAiRunCoachRun(garminActivity, userId) {
  try {
    const garminActivityTime = new Date(garminActivity.startTimeInSeconds * 1e3);
    ...
    for (const aiRunCoachRun of candidates) {
      ...
      const garminDistanceKm = (garminActivity.distanceInMeters || 0) / 1e3;
...
async function mergeGarminActivityWithAiRunCoachRun(aiRunCoachRunId, garminActivity, mergeCandidate, userId) {
  try {
    console.log(
    ...
    const enrichedData = {
      ...
      distance: (garminActivity.distanceInMeters || 0) / 1e3,
...
var HeartRateZones = class {
   ...
   */
  static getZoneRange(zoneNumber, maxHR) {
    const ranges = {
      1: { min: 0.5, max: 0.6 },
      2: { min: 0.6, max: 0.7 },
      3: { min: 0.7, max: 0.8 },
   ...
   */
  static getZoneDescription(zoneNumber) {
    const descriptions = {
      1: {
...
Return ONLY valid JSON in this exact format (no markdown, no extra text):
{
  ...
  const responseText = response.choices[0].message.content || '{"tone": "motivational"}';
  let parsed;
  try {
  ...
  return {
    ...
    toneReasoning: parsed.reasoning || "AI-determined optimal tone for this session"
  };
}
function buildAthleticProfile(userRecord) {
...
function buildSessionCharacteristics(request) {
  const workoutType = request.workoutType;
  const intensity = request.intensity;
  const goal = request.sessionGoal || "general fitness";
  const duration = request.duration ? `${Math.round(request.duration / 60)} minutes` : "unknown duration";
...
function buildFallbackBrief(workout) {
...
function buildFallbackStructure(workout) {
  if (workout.workoutType === "intervals" || workout.workoutType === "hill_repeats") {
    return {
      type: workout.workoutType,
      goal: workout.sessionGoal || "speed_development",
      phases: [
        { name: "warmup", durationKm: 1, durationSeconds: null, description: "Easy warm-up jog", repetitions: null },
...
async function generateTrainingPlan(userId, goalType, targetDistance, targetTime, targetDate, experienceLevel = "intermediate", daysPerWeek = 4, regularSessions = [], firstSessionStart = "flexible", durationWeeks, injuries = [], userTimezone = null, goalId = null, overrideAge = null, overrideGender = null, overrideHeight = null, overrideWeight = null) {
  try {
    ...
    const paceValues = recentRuns.map((r) => parsePaceSecs(r.avgPace)).filter((v) => v !== null);
    const avgPaceSecs = paceValues.length > 0 ? paceValues.reduce((a, b) => a + b, 0) / paceValues.length : null;
    const avgPaceStr = avgPaceSecs != null ? `${Math.floor(avgPaceSecs / 60)}:${String(Math.round(avgPaceSecs % 60)).padStart(2, "0")}/km` : null;
    const bestPaceSecs = paceValues.length > 0 ? Math.min(...paceValues) : null;
    const bestPaceStr = bestPaceSecs != null ? `${Math.floor(bestPaceSecs / 60)}:${String(Math.round(bestPaceSecs % 60)).padStart(2, "0")}/km` : null;
    const distanceTolerance = targetDistance * 0.2;
    const similarDistanceRuns = recentRuns.filter((r) => {
      const km = r.distance ? r.distance / 1e3 : r.distanceInMeters ? r.distanceInMeters / 1e3 : null;
      return km != null && Math.abs(km - targetDistance) <= distanceTolerance;
    });
    const goalDistanceTimes = similarDistanceRuns.map((r) => r.duration ? r.duration / 1e3 : null).filter((v) => v !== null && v > 0);
    const avgTimeAtGoalDistanceSecs = goalDistanceTimes.length > 0 ? goalDistanceTimes.reduce((a, b) => a + b, 0) / goalDistanceTimes.length : null;
    const avgTimeAtGoalDistanceStr = avgTimeAtGoalDistanceSecs != null ? `${Math.floor(avgTimeAtGoalDistanceSecs / 60)}:${String(Math.round(avgTimeAtGoalDistanceSecs % 60)).padStart(2, "0")}` : null;
    const bestTimeAtGoalSecs = goalDistanceTimes.length > 0 ? Math.min(...goalDistanceTimes) : null;
    const bestTimeAtGoalStr = bestTimeAtGoalSecs != null ? `${Math.floor(bestTimeAtGoalSecs / 60)}:${String(Math.round(bestTimeAtGoalSecs % 60)).padStart(2, "0")}` : null;
    const last3Runs = recentRuns.slice(0, 3);
    let daysSpanBetweenFirstAndLatest = null;
    if (last3Runs.length >= 2) {
      ...
      daysSpanBetweenFirstAndLatest = Math.floor(
        (latestRunDate.getTime() - firstRunDate.getTime()) / (1e3 * 60 * 60 * 24)
      );
    }
    ...
    const userHeight = user[0]?.height || overrideHeight || 170;
    const userWeight = user[0]?.weight || overrideWeight || 70;
    ...
    if (paceValues.length >= 6) {
      ...
      const diffSecs = oldestAvg - newestAvg;
    ...
    const weeksUntilTarget = durationWeeks && durationWeeks > 0 ? durationWeeks : targetDate ? Math.ceil((targetDate.getTime() - Date.now()) / (7 * 24 * 60 * 60 * 1e3)) : getPlanDuration(goalType, experienceLevel);
    console.log(`[Training Plan] Duration: durationWeeks=${durationWeeks}, weeksUntilTarget=${weeksUntilTarget}`);
    const userAge = user[0]?.dob ? Math.floor((Date.now() - new Date(user[0].dob).getTime()) / 315576e5) : overrideAge || 30;
...
- Average distance per run: ${last3AvgData ? (last3AvgData.avgDistance / 1e3).toFixed(2) : "0"}km
...
` : ""}
Schedule:
- Today is ${(/* @__PURE__ */ new Date()).toLocaleDateString("en-GB", { weekday: "long", year: "numeric", month: "long", day: "numeric" })} (day ${(/* @__PURE__ */ new Date()).getDay()} of the week, 0=Sun)
...
- focusAreas: should include "establish baseline", "build aerobic foundation", "injury prevention"`;
    ...
    if (planData.weeks.length < weeksUntilTarget) {
      ...
      while (planData.weeks.length < weeksUntilTarget) {
        ...
        planData.weeks.push({
          ...
          workouts: (lastWeek?.workouts || []).map((w) => ({ ...w }))
        });
      }
    ...
    for (const week of planData.weeks) {
      ...
      for (const workout of week.workouts) {
        const todayInUserTz = userTimezone ? /* @__PURE__ */ new Date((/* @__PURE__ */ new Date()).toLocaleDateString("en-CA", { timeZone: userTimezone }) + "T00:00:00") : /* @__PURE__ */ new Date();
        ...
        let hrZoneMinBpm = null;
        let hrZoneMaxBpm = null;
        let effortDescription = null;
        ...
        const plannedWorkoutResult = await db.insert(plannedWorkouts).values({
          weeklyPlanId,
          trainingPlanId: planId,
          ...
          hrZoneMinBpm,
          hrZoneMaxBpm,
          hrZoneScenario: hrZoneNumber ? hrZoneScenario : null,
          effortDescription,
          description: workout.description,
          instructions: workout.instructions,
          isCompleted: false,
          sessionGoal: workout.sessionGoal,
          // "build_fitness", "develop_speed", etc
...
RUNNER PROFILE:
- Age: ${userProfile?.dob ? Math.floor((Date.now() - new Date(userProfile.dob).getTime()) / (365.25 * 24 * 60 * 60 * 1e3)) : "Unknown"}
- Fitness Level: ${userProfile?.fitnessLevel || "Not specified"}
- Current CTL (Fitness): ${fitness?.ctl || "N/A"}
- Training Status: ${fitness?.status || "N/A"}

RECENT RUN (Just Completed):
- Distance: ${(run.distance / 1e3).toFixed(2)} km
...
var ACHIEVEMENT_CONFIGS = [
  {
    ...
    distanceKm: 1,
    toleranceUnderMeters: 50,
    toleranceOverMeters: 100,
  ...
  {
    type: "FASTEST_KM" /* FASTEST_KM */,
    distanceKm: 1,
    toleranceUnderMeters: 0,
    toleranceOverMeters: 0,
    title: "Fastest Km",
  ...
  {
    type: "PERSONAL_BEST_1_MILE" /* PERSONAL_BEST_1_MILE */,
    distanceKm: 1.60934,
    toleranceUnderMeters: 50,
    toleranceOverMeters: 100,
  ...
  {
    type: "FASTEST_MILE" /* FASTEST_MILE */,
    distanceKm: 1.60934,
    toleranceUnderMeters: 0,
    toleranceOverMeters: 0,
    title: "Fastest Mile",
  ...
  {
    type: "PERSONAL_BEST_5K" /* PERSONAL_BEST_5K */,
    distanceKm: 5,
    toleranceUnderMeters: 50,
    toleranceOverMeters: 100,
  ...
  {
    type: "PERSONAL_BEST_10K" /* PERSONAL_BEST_10K */,
    distanceKm: 10,
    toleranceUnderMeters: 50,
    toleranceOverMeters: 100,
  ...
  {
    type: "PERSONAL_BEST_HALF_MARATHON" /* PERSONAL_BEST_HALF_MARATHON */,
    distanceKm: 21.1,
    toleranceUnderMeters: 50,
    toleranceOverMeters: 100,
  ...
  {
    type: "PERSONAL_BEST_MARATHON" /* PERSONAL_BEST_MARATHON */,
    distanceKm: 42.2,
    toleranceUnderMeters: 50,
    toleranceOverMeters: 100,
...
function isWithinRange(actualKm, config) {
  const actualMeters = actualKm * 1e3;
  const targetMeters = config.distanceKm * 1e3;
...
function findFastestSegment(kmSplits, segmentKm) {
...
async function calculateRunAchievements(userId, runId, distance, avgPace, kmSplits = null) {
  const achievements2 = [];
  try {
    ...
    const pbConfigs = ACHIEVEMENT_CONFIGS.filter(
      (c) => c.type !== "FASTEST_KM" /* FASTEST_KM */ && c.type !== "FASTEST_MILE" /* FASTEST_MILE */
    );
    for (const config of pbConfigs) {
      if (!isWithinRange(distanceKm, config)) {
        continue;
      }
      let previousBestMinutes = null;
      ...
      if (previousBestMinutes === null || currentPaceMinutes < previousBestMinutes) {
        const improvementPercent = previousBestMinutes ? (previousBestMinutes - currentPaceMinutes) / previousBestMinutes * 100 : void 0;
        achievements2.push({
          type: config.type,
          title: config.title,
    ...
    if (achievements2.some((a) => a.type === "PERSONAL_BEST_5K" /* PERSONAL_BEST_5K */)) {
      const index2 = achievements2.findIndex((a) => a.type === "PERSONAL_BEST_10K" /* PERSONAL_BEST_10K */);
      if (index2 !== -1) {
        achievements2.splice(index2, 1);
      }
    }
    const fastestKmPace = findFastestSegment(kmSplits, 1);
    if (fastestKmPace) {
      let previousFastestKmPace = null;
      for (const prevRun of userRuns) {
        if (!prevRun.kmSplits) continue;
        const prevFastestKm = findFastestSegment(prevRun.kmSplits, 1);
      ...
      if (previousFastestKmPace === null || fastestKmPace < previousFastestKmPace) {
        achievements2.push({
          ...
          improvementPercent: previousFastestKmPace ? (previousFastestKmPace - fastestKmPace) / previousFastestKmPace * 100 : void 0
        });
      }
    }
    const fastestMilePace = findFastestSegment(kmSplits, 1.60934);
    if (fastestMilePace) {
      let previousFastestMilePace = null;
      for (const prevRun of userRuns) {
        if (!prevRun.kmSplits) continue;
      ...
      if (previousFastestMilePace === null || fastestMilePace < previousFastestMilePace) {
        achievements2.push({
          ...
          improvementPercent: previousFastestMilePace ? (previousFastestMilePace - fastestMilePace) / previousFastestMilePace * 100 : void 0
        });
      }
    }
    console.log(`[achievements] Calculated ${achievements2.length} achievements for run ${runId}`);
    return achievements2;
  } catch (error) {
...
var achievements_service_default = {
  ...
  try {
    ...
    if (changes.upcoming_workout_adjustments && Array.isArray(changes.upcoming_workout_adjustments)) {
      for (const adjustment of changes.upcoming_workout_adjustments) {
        if (adjustment.skip) {
          const updated = await db.update(plannedWorkouts).set({
            intensity: "rest",
...
async function declineAdaptation(adaptationId, userId) {
  try {
...
async function getPendingAdaptations(trainingPlanId, userId) {
  try {
...
var routes_adaptation_default = router2;
...
router3.get("/personal-bests", authMiddleware, async (req, res) => {
  try {
    ...
    const personalBests = await my_data_service_default.getPersonalBests(userId);
    res.json({
      success: true,
      data: { personalBests }
    });
...
router3.get("/statistics", authMiddleware, async (req, res) => {
  try {
    const userId = req.user?.userId;
    if (!userId) {
      return res.status(401).json({ error: "Unauthorized" });
    ...
    const stats = await my_data_service_default.getPeriodStatistics(userId, days);
    res.json({
      success: true,
      data: stats
    });
...
router3.get("/trends", authMiddleware, async (req, res) => {
  try {
    const userId = req.user?.userId;
    if (!userId) {
      return res.status(401).json({ error: "Unauthorized" });
    }
    const trends = await my_data_service_default.getDetailedTrends(userId, 30);
    res.json({
      success: true,
      data: trends
    });
...
router3.get("/detailed-trends", authMiddleware, async (req, res) => {
  try {
    const userId = req.user?.userId;
    if (!userId) {
      return res.status(401).json({ error: "Unauthorized" });
    ...
    const days = parseInt(req.query.days) || 30;
    const trends = await my_data_service_default.getDetailedTrends(userId, days);
    res.json({
      success: true,
      data: trends
    });
...
router3.get("/all-time-stats", authMiddleware, async (req, res) => {
  try {
    const userId = req.user?.userId;
    if (!userId) {
      return res.status(401).json({ error: "Unauthorized" });
    }
    const stats = await my_data_service_default.getAllTimeStats(userId);
    res.json({
      success: true,
      data: stats
    });
...
router4.post("/runs/:runId/achievements", authMiddleware, async (req, res) => {
  try {
    ...
    const run = runData[0];
    const achievements2 = await achievements_service_default.calculateRunAchievements(
      userId,
      runId,
      run.distance || 0,
      run.avgPace,
...
router4.get("/users/:userId/achievements", authMiddleware, async (req, res) => {
  try {
...
var routes_achievements_default = router4;
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        await db.insert(coachingSessionEvents2).values({
          runId,
          plannedWorkoutId,
          eventType,
          eventPhase,
...
async function findScenicFeatures(lat, lng, radiusMeters) {
  try {
    const query = `
      [out:json][timeout:10];
      (
      ...
      out center;
    `;
    const response = await axios2.post(OVERPASS_API_URL, `data=${encodeURIComponent(query)}`, {
    ...
    for (const el of elements) {
      ...
      const bearing = calculateBearing2([lng, lat], [elLng, elLat]);
      const distance = getDistanceKm({ lat, lng }, { lat: elLat, lng: elLng });
      if (distance < 0.2 || distance > radiusMeters / 1e3) continue;
      features.push({ type, name, lat: elLat, lng: elLng, bearing, distance });
    }
    console.log(`\u{1F333} Found ${features.length} scenic features within ${radiusMeters}m (${elements.length} raw OSM elements)`);
...
function selectScenicWaypoints(features, startLat, startLng, targetDistanceKm) {
  ...
  const pool2 = inRange.length >= 3 ? inRange : features;
  const typePriority = {
    park: 5,
    river: 4,
    trail: 4,
  ...
  const sortedPool = [...pool2].sort((a, b) => (typePriority[b.type] || 0) - (typePriority[a.type] || 0));
  const quadrants = [null, null, null, null];
  ...
  const selectedFeatures = quadrants.filter(Boolean);
...
function analyzeRoadClasses(roadClassDetails) {
  ...
  const terrainScore = Math.max(0, Math.min(1, (tp + pp) * 1.5 - rp * 0.5 - hp * 2));
...
function validateRoute(coordinates, actualDistanceMeters, targetDistanceMeters, roadClassDetails) {
  ...
  for (let i = 1; i < sampled.length - 1; i++) {
    if (calculateAngle2(sampled[i - 1], sampled[i], sampled[i + 1]) > 150) {
      issues.push({ type: "U_TURN", location: sampled[i], severity: "HIGH" });
    }
  }
  const segSet = /* @__PURE__ */ new Set();
...
function calculateAngle2(p1, p2, p3) {
  const b1 = calculateBearing2(p1, p2), b2 = calculateBearing2(p2, p3);
  let a = Math.abs(b2 - b1);
  if (a > 180) a = 360 - a;
  return a;
}
...
function calculateCompactness(coordinates, startLat, startLng) {
  ...
  let area = 0;
  const n = coordinates.length, cosLat = Math.cos(startLat * Math.PI / 180);
  for (let i = 0; i < n; i++) {
    const j = (i + 1) % n;
  ...
  if (perim === 0) return 0;
  return Math.min(1, 4 * Math.PI * area / (perim * perim) / 0.1);
}
function calculateAngularSpread(coordinates, startLat, startLng) {
  if (coordinates.length < 10) return 0;
  const sp = [startLng, startLat], cosLat = Math.cos(startLat * Math.PI / 180);
  ...
  for (let i = step; i < coordinates.length - step; i += step) {
    ...
    if (Math.sqrt(dx * dx + dy * dy) > 100) bearings.push(calculateBearing2(sp, coordinates[i]));
  }
  if (bearings.length < 3) return 0;
  const sectors = /* @__PURE__ */ new Set();
...
function calculateProximityOverlap(coordinates, startLat, startLng) {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    for (let j = i + 1; j < s.length; j++) {
      if (Math.abs(s[j].p - s[i].p) < 0.3) continue;
...
function calculateBacktrackRatio(coordinates) {
  if (coordinates.length < 10) return 0;
  const gs = 3e-4, segSet = /* @__PURE__ */ new Set();
  let tot = 0, bt = 0;
  for (let i = 0; i < coordinates.length - 1; i++) {
    ...
    if (g1 !== g2) {
      tot++;
      if (segSet.has(`${g2}-${g1}`)) bt++;
      segSet.add(`${g1}-${g2}`);
    }
  }
  return tot > 0 ? bt / tot : 0;
}
function calculateRouteDiversity(a, b, startLat, startLng) {
  const sp = [startLng, startLat];
  function dom(coords) {
    const s = Math.floor(coords.length * 0.2), e = Math.floor(coords.length * 0.6), step = Math.max(1, Math.floor((e - s) / 10));
    let ss = 0, cs = 0, c = 0;
    for (let i = s; i < e; i += step) {
      const br = calculateBearing2(sp, coords[i]);
      ss += Math.sin(br * Math.PI / 180);
      cs += Math.cos(br * Math.PI / 180);
      c++;
    }
    return c === 0 ? 0 : (Math.atan2(ss / c, cs / c) * 180 / Math.PI + 360) % 360;
  }
  let d = Math.abs(dom(a) - dom(b));
  if (d > 180) d = 360 - d;
  return d / 180;
}
...
var LOOSE_THRESHOLDS = {
  loopQuality: 0.4,
  loopQualityScenic: 0.3,
  backtrackRatio: 0.4,
  proximityOverlap: 0.4,
  compactness: 0.08,
  angularSpread: 0.15
};
...
async function evaluateCandidates(allResults, latitude, longitude, distanceMeters, thresholds, tierLabel) {
  const accepted = [];
  const rejected = [];
  for (const result of allResults) {
    const { seed, ghResponse, isScenic } = result;
    if (!ghResponse?.paths?.length) continue;
    const path3 = ghResponse.paths[0];
    let coordinates = path3.points.coordinates;
    const label = `[${tierLabel}] ${isScenic ? `Scenic(${seed})` : `Seed ${seed}`}`;
    const distanceDiffPercent = Math.abs(path3.distance - distanceMeters) / distanceMeters;
    if (distanceDiffPercent > 0.15) {
      console.log(`${label}: Rejected - distance ${(path3.distance / 1e3).toFixed(2)}km is ${(distanceDiffPercent * 100).toFixed(1)}% off target ${(distanceMeters / 1e3).toFixed(1)}km (max \xB115%)`);
    ...
    console.log(`${label}: Loop=${loopQuality.toFixed(2)}, Backtrack=${backtrackRatio.toFixed(2)}, Dist=${(path3.distance / 1e3).toFixed(2)}km (${(distanceDiffPercent * 100).toFixed(1)}% off)`);
    ...
    if (loopQuality < minLoop) {
      console.log(`${label}: Rejected - poor loop (${loopQuality.toFixed(2)} < ${minLoop})`);
      rejected.push({ raw: result, metrics });
      continue;
    }
    if (backtrackRatio > thresholds.backtrackRatio) {
      console.log(`${label}: Rejected - backtracking (${backtrackRatio.toFixed(2)} > ${thresholds.backtrackRatio})`);
      rejected.push({ raw: result, metrics });
      continue;
    }
    if (proximityOverlap > thresholds.proximityOverlap) {
      console.log(`${label}: Rejected - overlap (${proximityOverlap.toFixed(2)} > ${thresholds.proximityOverlap})`);
      rejected.push({ raw: result, metrics });
      continue;
    }
    if (!isScenic && compactness < thresholds.compactness) {
      console.log(`${label}: Rejected - elongated (${compactness.toFixed(2)} < ${thresholds.compactness})`);
      rejected.push({ raw: result, metrics });
      continue;
    }
    if (!isScenic && angularSpread < thresholds.angularSpread) {
      console.log(`${label}: Rejected - poor spread (${angularSpread.toFixed(2)} < ${thresholds.angularSpread})`);
      rejected.push({ raw: result, metrics });
      continue;
    }
    ...
    const roadClassDetails = path3.details?.road_class || [];
    ...
    const validation = validateRoute(coordinates, path3.distance, distanceMeters, roadClassDetails);
    if (!validation.isValid) {
      console.log(`${label}: Rejected - invalid`);
      rejected.push({ raw: result, metrics });
      continue;
    }
    const popularityScore = await getRoutePopularityScore(coordinates);
    accepted.push({ route: path3, validation, popularityScore, terrainScore: roadAnalysis.terrainScore, loopQuality, backtrackRatio, compactness, angularSpread, proximityOverlap, coordinates, isScenic });
...
async function generateIntelligentRoute(request) {
  ...
  if (scenicWaypoints.length > 0) {
    console.log(`\u{1F333} Selected ${scenicWaypoints.length} scenic waypoints:`);
...
\u2501\u2501\u2501 TIER 1: Hike profile + strict thresholds \u2501\u2501\u2501`);
  ...
  console.log(`\u{1F4CA} Tier 1: ${tier1.accepted.length} accepted, ${tier1.rejected.length} rejected`);
...
\u2501\u2501\u2501 TIER 2: Re-evaluating ${tier1.rejected.length} rejected hike candidates with loose thresholds \u2501\u2501\u2501`);
    ...
    console.log(`\u{1F4CA} Tier 2: ${tier2.accepted.length} more accepted (total: ${allCandidates.length})`);
  }
...
\u2501\u2501\u2501 TIER 3: Foot profile fallback + strict thresholds \u2501\u2501\u2501`);
    ...
    console.log(`\u{1F4CA} Tier 3: ${tier3.accepted.length} more accepted (total: ${allCandidates.length})`);
...
\u2501\u2501\u2501 TIER 3b: Re-evaluating foot candidates with loose thresholds \u2501\u2501\u2501`);
      ...
      console.log(`\u{1F4CA} Tier 3b: ${tier3b.accepted.length} more accepted (total: ${allCandidates.length})`);
    }
  }
  if (allCandidates.length === 0) {
    throw new NoRoutesError(
      `We couldn't find any suitable running routes for a ${distanceKm}km run at this location. This area may not have enough paths, trails, or roads for a circular route. Try adjusting the distance or moving to a location with more connected paths.`
    );
  }
  console.log(`
\u{1F4CA} Total: ${allCandidates.length} valid candidates (${allCandidates.filter((c) => c.isScenic).length} scenic, ${allCandidates.filter((c) => !c.isScenic).length} round-trip)`);
  const scored = allCandidates.map((c) => {
    ...
    const totalScore = Math.min(1, scenicScore) * 0.2 + shapeScore * 0.2 + c.validation.qualityScore * 0.2 + (1 - c.backtrackRatio) * 0.15 + c.loopQuality * 0.15 + c.popularityScore * 0.1;
    console.log(`  ${c.isScenic ? "\u{1F33F}" : "\u{1F504}"} scenic=${scenicScore.toFixed(2)}, shape=${shapeScore.toFixed(2)}, quality=${c.validation.qualityScore.toFixed(2)} \u2192 ${totalScore.toFixed(3)}`);
  ...
  for (const c of scored) {
    ...
    for (const s of selected) {
      if (calculateRouteDiversity(c.coordinates, s.coordinates, latitude, longitude) < 0.15) {
        diverse = false;
        break;
      }
    }
  ...
  return selected.map((c, i) => {
    ...
    return {
      ...
      qualityScore: c.validation.qualityScore,
      loopQuality: c.loopQuality,
      backtrackRatio: c.backtrackRatio,
...
function encodeValue(value) {
  ...
  while (n >= 32) {
    enc += String.fromCharCode((32 | n & 31) + 63);
    n >>= 5;
  }
  enc += String.fromCharCode(n + 63);
  return enc;
}
...
async function registerRoutes(app2) {
  ...
  app2.use("/api", routes_adaptation_default);
  ...
  app2.get("/api/friend-requests/:userId", authMiddleware, async (req, res) => {
    try {
      ...
      const sent = toRows(sentResult).map((r) => ({
        id: r.id,
        ...
        addresseeName: r.addressee_name ?? null,
        addresseeProfilePic: r.addressee_profile_pic ?? null,
        requesterName: null,
        requesterProfilePic: null
      }));
      const received = toRows(receivedResult).map((r) => ({
        id: r.id,
        requesterId: r.requester_id,
        addresseeId: r.addressee_id,
        status: r.status,
        ...
        requesterProfilePic: r.requester_profile_pic ?? null,
        addresseeName: null,
  ...
  app2.post("/api/test/coaching-plan-reminder", async (req, res) => {
    try {
      ...
      const { sendCoachingPlanReminder: sendCoachingPlanReminder2 } = await Promise.resolve().then(() => (init_notification_service(), notification_service_exports));
      const result = await sendCoachingPlanReminder2(
        user.id,
        workoutName,
        distance ? parseFloat(distance) : void 0,
        intensity
      );
  ...
  app2.get("/api/invite/:userId", async (req, res) => {
    try {
      const { userId } = req.params;
  ...
  app2.get("/api/users/search", optionalAuthMiddleware, async (req, res) => {
    try {
      ...
      console.log(`[Search] Found ${users2.length} users for query "${query}"`);
      const enriched = await Promise.all(users2.map(async (u) => {
        ...
        if (currentUserId && currentUserId !== u.id) {
          ...
          if (outgoingRequests) {
            friendRequestStatus = outgoingRequests.status;
            friendRequestId = outgoingRequests.id;
          } else if (incomingRequests) {
            friendRequestStatus = `received_${incomingRequests.status}`;
  ...
  app2.get("/api/users/me", authMiddleware, async (req, res) => {
    try {
  ...
  app2.post("/api/users/me/fcm-token", authMiddleware, async (req, res) => {
    try {
  ...
  app2.get("/api/friend-requests/:userId/debug", authMiddleware, async (req, res) => {
    try {
      const { userId } = req.params;
  ...
  app2.post("/api/friend-requests/:id/withdraw", authMiddleware, async (req, res) => {
    try {
      const { id } = req.params;
      const requesterId = req.user.userId;
  ...
  function normalizeDistanceMeters(run) {
  ...
  function normalizeDurationSeconds(run) {
    const rawDur = run.duration || 0;
    return rawDur > 86400 ? Math.round(rawDur / 1e3) : rawDur;
  }
  function transformRunForAndroid(run) {
    ...
    const durationMs = rawDuration > 86400 ? rawDuration : rawDuration * 1e3;
  ...
  app2.get("/api/users/:userId/run-history-stats", authMiddleware, async (req, res) => {
    try {
      ...
      res.json({
        ...
        bestPaceFormatted: bestPaceSec ? secToFormatted(bestPaceSec) : void 0,
        avgDistanceKm: Math.round(avgDistanceKm * 10) / 10,
        avgCadence,
        avgHeartRate,
        consistencyTrend,
        lastRunPaceFormatted: lastRunPace ? secToFormatted(lastRunPace) : void 0,
  ...
  app2.post("/api/runs", authMiddleware, async (req, res) => {
    try {
      ...
      const durationInSeconds = rawDuration > 86400 ? Math.round(rawDuration / 1e3) : rawDuration;
      ...
      if ((totalSteps === 0 || !totalSteps) && runData.cadence && durationInSeconds) {
        ...
        console.log(`[POST /api/runs] Calculated totalSteps: ${runData.cadence} steps/min \xD7 ${durationMinutes.toFixed(1)} min = ${totalSteps} steps`);
      }
      const processedRunData = {
        ...runData,
        duration: durationInSeconds,
        totalSteps,
        // Use calculated or provided value
  ...
  app2.post("/api/runs/:id/comprehensive-analysis", authMiddleware, async (req, res) => {
    try {
      ...
      if (existingAnalysis?.analysis) {
        let cachedAnalysis = existingAnalysis.analysis;
        if (cachedAnalysis.analysis && typeof cachedAnalysis.analysis === "object" && cachedAnalysis.analysis.performanceScore !== void 0) {
          cachedAnalysis = cachedAnalysis.analysis;
        }
        if (cachedAnalysis.performanceScore !== void 0) {
      ...
      const analysis = await aiService.generateComprehensiveRunAnalysis({
        ...
        linkedPlanId: run.linkedPlanId || void 0,
        planProgressWeek: run.planProgressWeek || void 0,
        planProgressWeeks: run.planProgressWeeks || void 0,
        workoutType: run.workoutType || void 0,
  ...
  app2.post("/api/runs/:id/freeform-analysis", authMiddleware, async (req, res) => {
    try {
      const runId = req.params.id;
      const userId = req.user.userId;
...
Run data:
${contextJson}` : `Provide a brief, insightful analysis of this run. Highlight what went well and one area to improve.
...
${contextJson}`;
  ...
  app2.post("/api/routes/generate-options", authMiddleware, async (req, res) => {
    try {
      ...
      const formattedRoutes = routes2.map((route, index2) => ({
        id: route.id,
        name: route.name,
  ...
  app2.get("/api/goals/:userId", authMiddleware, async (req, res) => {
    try {
      ...
      const goals2 = rawGoals.map((goal) => ({
        id: goal.id,
        userId: goal.userId,
        type: goal.type,
        title: goal.title,
        ...
        startingWeightKg: goal.startingWeightKg ?? null,
        currentProgress: goal.progressPercent ?? 0,
        isActive: goal.status === "active",
        isCompleted: !!goal.completedAt,
        relatedRunSessionIds: goal.relatedRunSessionIds ?? [],
        createdAt: goal.createdAt?.toISOString(),
        updatedAt: goal.updatedAt?.toISOString(),
        completedAt: goal.completedAt?.toISOString()
      }));
      console.log(`[GET /api/goals/:userId] Returning ${goals2.length} formatted goals`);
      res.json(goals2);
    } catch (error) {
  ...
  app2.post("/api/goals", authMiddleware, async (req, res) => {
    try {
      ...
      const goal = {
        ...
        targetWeightKg: rawGoal.targetWeightKg ?? null,
        startingWeightKg: rawGoal.startingWeightKg ?? null,
        currentProgress: rawGoal.progressPercent ?? 0,
        isActive: rawGoal.status === "active",
        isCompleted: !!rawGoal.completedAt,
        relatedRunSessionIds: rawGoal.relatedRunSessionIds ?? [],
  ...
  app2.put("/api/goals/:id", authMiddleware, async (req, res) => {
    try {
      ...
      if (req.body.isCompleted === true) {
        ...
        updateData.completedAt = req.body.completedAt ? new Date(req.body.completedAt) : /* @__PURE__ */ new Date();
  ...
  app2.post("/api/ai/tts", async (req, res) => {
    try {
      const { text: text2, voice, coachAccent, coachGender } = req.body;
      const aiService = await Promise.resolve().then(() => (init_ai_service(), ai_service_exports));
      const audioBuffer = await aiService.generateTTS(text2, voice || "alloy", void 0, coachAccent, coachGender);
  ...
  app2.delete("/api/connected-devices/:deviceId", authMiddleware, async (req, res) => {
    try {
      ...
      console.log(`\u{1F50C} Disconnect request for device: ${deviceId}, user: ${req.user.userId}`);
      let device2 = await storage.getConnectedDevice(deviceId);
      if (!device2) {
        const devices = await storage.getConnectedDevices(req.user.userId);
        device2 = devices.find((d) => String(d.id) === String(deviceId)) || void 0;
        console.log(`\u{1F50D} Fallback search found: ${device2 ? device2.id : "nothing"}, checked ${devices.length} devices: [${devices.map((d) => `${d.id}(${typeof d.id})`).join(", ")}]`);
      }
      if (!device2) {
        return res.status(404).json({ error: "Device not found" });
      }
      if (device2.userId !== req.user.userId) {
        return res.status(403).json({ error: "Not authorized" });
      }
  ...
  app2.post("/api/device-data/sync", authMiddleware, async (req, res) => {
    try {
      ...
      const devices = await storage.getConnectedDevices(req.user.userId);
      const device2 = devices.find((d) => d.deviceType === deviceType && d.isActive);
      if (device2) {
        await storage.updateConnectedDevice(device2.id, { lastSyncAt: /* @__PURE__ */ new Date() });
      }
  ...
  app2.get("/api/auth/garmin/callback", async (req, res) => {
    ...
    try {
      ...
      if (state) {
        try {
          oauthStateRecord = await storage.claimOauthState(state);
          if (!oauthStateRecord) {
          ...
          if (/* @__PURE__ */ new Date() > new Date(oauthStateRecord.expiresAt)) {
            console.error("[SECURITY] Garmin callback - state expired:", { state, expiredAt: oauthStateRecord.expiresAt });
            const errorUrl = appRedirectUrl.includes("?") ? `${appRedirectUrl}&garmin=error&message=state_expired` : `${appRedirectUrl}?garmin=error&message=state_expired`;
          ...
          if (oauthStateRecord.provider !== "garmin") {
            ...
            const errorUrl = appRedirectUrl.includes("?") ? `${appRedirectUrl}&garmin=error&message=invalid_provider` : `${appRedirectUrl}?garmin=error&message=invalid_provider`;
            return res.redirect(errorUrl);
          }
          ...
          const errorUrl = appRedirectUrl.includes("?") ? `${appRedirectUrl}&garmin=error&message=state_validation_error` : `${appRedirectUrl}?garmin=error&message=state_validation_error`;
          return res.redirect(errorUrl);
        }
      } else {
      ...
      try {
        ...
        if (state) {
          const stateRecord = await storage.getOauthState(state);
          if (stateRecord) {
  ...
  app2.post("/api/runs/:runId/enrich-with-garmin-data", authMiddleware, async (req, res) => {
    try {
      const { runId } = req.params;
      const devices = await storage.getConnectedDevices(req.user.userId);
      const garminDevice = devices.find((d) => d.deviceType === "garmin" && d.isActive);
      ...
      if (refreshToken) {
        try {
          console.log("\u{1F504} Refreshing Garmin access token before enrichment...");
          ...
          const isInvalidGrant = refreshError.message?.includes("invalid_grant") || refreshError.message?.includes("Invalid refresh token") || refreshError.message?.includes("400");
          if (isInvalidGrant) {
      ...
      const runStartTimeSec = Math.floor(runStartTimeMs / 1e3);
      const searchStartSec = runStartTimeSec - THIRTY_MINS_S;
      const searchEndSec = runStartTimeSec + THIRTY_MINS_S;
      console.log(`[Garmin Enrich] \u{1F50D} Matching Garmin activity by START TIME (\xB130 mins)`);
      ...
      if (localActivity) {
        ...
        console.log(`[Garmin Enrich]    Started at: ${new Date(localActivity.startTimeInSeconds * 1e3).toISOString()}`);
      ...
      res.json({
        ...
        garminActivity: {
          ...
          startTime: matchingActivity.startTime || (matchingActivity.startTimeInSeconds ? new Date(matchingActivity.startTimeInSeconds * 1e3).toISOString() : null)
        }
      });
      setImmediate(() => {
        (async () => {
  ...
  app2.get("/api/garmin/permissions", authMiddleware, async (req, res) => {
    try {
      const { getCurrentPermissions: getCurrentPermissions2 } = await Promise.resolve().then(() => (init_garmin_permissions_service(), garmin_permissions_service_exports));
      const permissions = await getCurrentPermissions2(req.user.id);
  ...
  app2.post("/api/garmin/reauthorize", authMiddleware, async (req, res) => {
    try {
      const { getReauthorizationUrl: getReauthorizationUrl2 } = await Promise.resolve().then(() => (init_garmin_permissions_service(), garmin_permissions_service_exports));
      const baseUrl = process.env.APP_URL || `${req.protocol}://${req.get("host")}`;
      const authUrl = await getReauthorizationUrl2(req.user.id, baseUrl);
      res.json({ authUrl });
    } catch (error) {
  ...
  app2.post("/api/garmin/disconnect", authMiddleware, async (req, res) => {
    try {
      ...
      await disconnectDevice2(req.user.id);
  ...
  app2.get("/api/garmin/user-permissions/:garminUserId", async (req, res) => {
    try {
      ...
      res.status(200).json({
        ...
        status: "active",
        connectedAt: device2.createdAt,
        lastSync: device2.lastSyncAt
      });
  ...
  const garminWebhook = (path3, handler) => {
    app2.post(`/api/garmin/webhook/${path3}`, handler);
    app2.post(`/api/garmin/webhooks/${path3}`, handler);
  };
    ...
    try {
      ...
      for (const activity of activities) {
        ...
        try {
          ...
          console.log(`[Garmin Webhook] Processing activity for user ${device2.userId}: ${activity.activityName || activityType} (type: ${activityType})`);
          await storage.updateGarminWebhookEvent(webhookEventId, {
            userId: device2.userId
          });
          ...
          if (isRunOrWalk && activity.distanceInMeters > 0) {
            ...
            if (mergeCandidate && mergeCandidate.matchScore > 50) {
              console.log(`[Garmin Webhook] \u{1F517} Fuzzy matched to existing run ${mergeCandidate.aiRunCoachRunId}`);
            ...
            setImmediate(() => {
              (async () => {
                try {
                  ...
                  await reassessTrainingPlansWithRunData(device2.userId, runId);
                } catch (err) {
          ...
          await db.insert(webhookFailureQueue).values({
            ...
            nextRetryAt: new Date(Date.now() + 10 * 60 * 1e3)
            // Retry in 10 minutes
          }).catch((queueError) => {
  ...
  function determineTerrain(activityType) {
  ...
  garminWebhook("activity-details", async (req, res) => {
    try {
      console.log("[Garmin Webhook] Received activity details push:", JSON.stringify(req.body).slice(0, 1e3));
      res.status(200).json({ success: true });
        ...
        try {
          ...
          if (existingActivity) {
            ...
            await db.update(garminActivities).set({
              samples: samples.length > 0 ? processedSamples : null,
              // Store time series
  ...
  garminWebhook("sleeps", async (req, res) => {
    try {
      ...
      for (const sleep of sleeps) {
        try {
          ...
          const deepScore = Math.min(deepPercent * 1.5, 30);
          const remScore = Math.min(remPercent * 1.2, 25);
          const restlessnessScore = sleep.sleepScores?.restlessness?.qualifierKey === "EXCELLENT" ? 20 : sleep.sleepScores?.restlessness?.qualifierKey === "GOOD" ? 15 : 10;
          const awakeScore = sleep.sleepScores?.awakeCount?.qualifierKey === "EXCELLENT" ? 15 : sleep.sleepScores?.awakeCount?.qualifierKey === "GOOD" ? 10 : 5;
          const sleepData = {
            userId,
            date,
            ...
            validationType: sleep.validation,
            // AUTO_FINAL, MANUAL, etc.
          ...
          await db.insert(webhookFailureQueue).values({
            ...
            nextRetryAt: new Date(Date.now() + 30 * 60 * 1e3)
          }).catch((queueError) => {
  ...
  garminWebhook("stress", async (req, res) => {
    try {
  ...
  garminWebhook("hrv", async (req, res) => {
    try {
      ...
      for (const hrv of hrvArray) {
        try {
          ...
          const device2 = await findUserByGarminToken(userAccessToken);
          if (!device2) {
          ...
          console.log(`[Garmin Webhook] Processing HRV for user ${device2.userId}, date: ${date}`);
          let weeklyAvg = hrv.weeklyAvg;
          let statusQualifier = hrv.hrvStatus;
          if (hrv.hrvValues && Object.keys(hrv.hrvValues).length > 0) {
            const hrvReadingValues = Object.values(hrv.hrvValues);
          ...
          const hrvFields = {
            ...
            // Time range
            hrvStartTimeGMT: hrv.startTimeGMT || new Date((hrv.startTimeInSeconds || 0) * 1e3).toISOString(),
            hrvEndTimeGMT: hrv.endTimeGMT || new Date(((hrv.startTimeInSeconds || 0) + (hrv.durationInSeconds || 0)) * 1e3).toISOString(),
  ...
  garminWebhook("dailies", async (req, res) => {
    try {
      ...
      for (const daily of dailies) {
        try {
          ...
          console.log(`[Garmin Webhook] Processing daily for user ${device2.userId}, date: ${date}`);
          const dailyFields = {
            // Metadata
            ...
            pushDistanceInMeters: daily.pushDistanceInMeters,
            // Wheelchair metric
            ...
            // Duration breakdown
            durationInSeconds: daily.durationInSeconds,
            // Total time awake
            activeTimeInSeconds: daily.activeTimeInSeconds,
            // Time in activity
            ...
            stressDurationInSeconds: daily.stressDurationInSeconds,
            // Total stress time
            restStressDurationInSeconds: daily.restStressDurationInSeconds,
            // Rest stress
            activityStressDurationInSeconds: daily.activityStressDurationInSeconds,
            // Activity stress
  ...
  garminWebhook("body-compositions", async (req, res) => {
    try {
      ...
      for (const comp of compositions) {
        ...
        if (device2) {
          ...
          console.log(`[Garmin Webhook] Processing body composition for user ${device2.userId}, date: ${date}`);
  ...
  garminWebhook("pulse-ox", async (req, res) => {
    try {
  ...
  garminWebhook("respiration", async (req, res) => {
    try {
      ...
      for (const resp of respirations) {
        try {
          ...
          const respirationValues = Object.values(resp.timeOffsetEpochToBreaths || {});
          let minResp = 50;
          let maxResp = 0;
          let avgResp = 0;
          ...
          const isDuringSleep = avgResp < 13;
          const respFields = {
            avgWakingRespirationValue: !isDuringSleep ? avgResp : void 0,
            avgSleepRespirationValue: isDuringSleep ? avgResp : void 0,
  ...
  app2.get("/api/garmin/webhook/deregistrations", (_req, res) => {
    res.status(200).json({ success: true });
  });
  app2.get("/api/garmin/webhooks/deregistrations", (_req, res) => {
    res.status(200).json({ success: true });
  });
  garminWebhook("deregistrations", async (req, res) => {
    try {
        ...
        try {
          ...
          try {
            await sendNotification({
              userId,
              title: "\u{1F50C} Garmin Connection Removed",
          ...
          await db.insert(webhookFailureQueue).values({
            ...
            userId: dereg.userAccessToken ? "unknown" : void 0,
            payload: dereg,
  ...
  app2.get("/api/garmin/webhooks/queue/stats", authMiddleware, async (req, res) => {
    try {
      ...
      const { getWebhookQueueStats: getWebhookQueueStats2 } = await Promise.resolve().then(() => (init_webhook_processor(), webhook_processor_exports));
      const stats = await getWebhookQueueStats2();
      res.json({
        success: true,
        data: stats,
        timestamp: (/* @__PURE__ */ new Date()).toISOString()
  ...
  app2.post("/api/garmin/webhooks/queue/retry/:webhookId", authMiddleware, async (req, res) => {
    try {
      ...
      const { retryWebhook: retryWebhook2 } = await Promise.resolve().then(() => (init_webhook_processor(), webhook_processor_exports));
      const success = await retryWebhook2(webhookId);
      if (!success) {
  ...
  app2.post("/api/garmin/webhooks/queue/process", authMiddleware, async (req, res) => {
    try {
      ...
      const { processWebhookFailureQueue: processWebhookFailureQueue2 } = await Promise.resolve().then(() => (init_webhook_processor(), webhook_processor_exports));
      const result = await processWebhookFailureQueue2();
      res.json({
        success: true,
        data: result,
  ...
  app2.post("/api/garmin/webhook-test", authMiddleware, async (req, res) => {
    try {
      ...
      const testActivities = activities.map((activity, index2) => ({
        ...activity,
        userAccessToken: `test_token_${req.user.userId}_${index2}`,
        activityId: activity.activityId || `test_${Date.now()}_${index2}`
      }));
      const { garminWebhook: garminWebhook2 } = await import("./garmin-webhook-handler");
      const mockReq = {
      ...
      await garminWebhook2("activities", mockReq, mockRes);
      const eventIds = await Promise.all(
        testActivities.map(async (activity) => {
          const events2 = await storage.getRecentGarminWebhookEvents(req.user.userId, 1);
          return events2[0]?.id;
        })
      );
      res.json({
  ...
  app2.get("/api/garmin/webhook-stats", authMiddleware, async (req, res) => {
    try {
      ...
      const matchRate7d = stats7d.totalReceived > 0 ? (stats7d.totalMerged / stats7d.totalReceived * 100).toFixed(1) : "0.0";
      res.json({
        success: true,
        stats: {
          last24h: stats24h,
  ...
  app2.get("/api/garmin/new-activities", authMiddleware, async (req, res) => {
    try {
      const since = req.query.since;
      const limit = Math.min(parseInt(req.query.limit || "20", 10), 50);
      const events2 = await storage.getRecentGarminWebhookEvents(req.user.userId, limit * 2);
      let filteredEvents = events2.filter(
        (e) => e.status === "created_run" || e.status === "merged_run"
      );
      if (since) {
        const sinceDate = new Date(since);
        filteredEvents = filteredEvents.filter(
          (e) => e.createdAt && new Date(e.createdAt) > sinceDate
        );
      }
  ...
  garminWebhook("permissions", async (req, res) => {
    try {
      const { handlePermissionChange: handlePermissionChange2 } = await Promise.resolve().then(() => (init_garmin_permissions_service(), garmin_permissions_service_exports));
  ...
  const garminAckHandler = async (_req, res) => {
    res.status(200).json({ success: true });
  };
  garminWebhook("moveiq", async (req, res) => {
    try {
      console.log("[Garmin Webhook] Received MoveIQ push:", JSON.stringify(req.body).slice(0, 1e3));
      res.status(200).json({ success: true });
          ...
          for (const device2 of devices) {
            ...
            if (existingMoveiq) {
              ...
              await db.insert(garminMoveIQ).values({
                userId: device2.userId,
                runId: matchingActivity.runId || void 0,
  ...
  garminWebhook("blood-pressure", async (req, res) => {
    try {
      console.log("[Garmin Webhook] Received blood pressure push:", JSON.stringify(req.body).slice(0, 1e3));
      res.status(200).json({ success: true });
  ...
  function classifyBloodPressure2(systolic, diastolic) {
    if (systolic < 120 && diastolic < 80) return "OPTIMAL";
  ...
  garminWebhook("epochs", async (req, res) => {
    try {
      ...
      const epochDate = epochTimestamp ? new Date(epochTimestamp * 1e3).toISOString().split("T")[0] : (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
      for (const device2 of devices) {
        try {
          ...
          if (existingAggregate?.compressedAt) {
            console.log(`[Garmin Webhook] Epochs already compressed for ${device2.userId} on ${epochDate}, skipping`);
            continue;
          }
          ...
          if (existingAggregate) {
            await db.update(garminEpochsAggregate).set({
            ...
            console.log(`[Garmin Webhook] Created epochs aggregate for ${device2.userId}, date: ${epochDate} (${epochs.length} epochs)`);
          }
        } catch (epochError) {
  ...
  garminWebhook("health-snapshot", async (req, res) => {
    try {
      ...
      for (const snapshot of snapshots) {
        try {
          const snapshotDate = snapshot.calendarDate || new Date((snapshot.startTimeInSeconds || 0) * 1e3).toISOString().split("T")[0];
          let userId;
          for (const device2 of devices) {
  ...
  garminWebhook("user-metrics", async (req, res) => {
    try {
      console.log("[Garmin Webhook] Received user-metrics push:", JSON.stringify(req.body).slice(0, 1e3));
      res.status(200).json({ success: true });
      ...
      for (const metrics of metricsData) {
        try {
          const date = metrics.calendarDate || (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
          let userId;
          for (const device2 of devices) {
            const recentRun = await db.query.runs.findFirst({
              where: and14(
          ...
          if (ageDifference < -10) {
            ...
            fitnessAgeCategory = "NORMAL";
          } else {
            fitnessAgeCategory = "BELOW_AVERAGE";
          }
          const metricsFields = {
            userId,
            date,
  ...
  garminWebhook("skin-temperature", async (req, res) => {
    try {
      console.log("[Garmin Webhook] Received skin temperature push:", JSON.stringify(req.body).slice(0, 1e3));
      res.status(200).json({ success: true });
      ...
      for (const temp of temperatureData) {
        try {
          const date = temp.calendarDate || new Date((temp.startTimeInSeconds || 0) * 1e3).toISOString().split("T")[0];
          let userId;
          for (const device2 of devices) {
            const recentRun = await db.query.runs.findFirst({
              where: and14(
          ...
          if (existing) {
            ...
            console.log(`[Garmin Webhook] Created skin temperature for ${date}: deviation=${deviationValue.toFixed(2)}\xB0C (calculated=${calculatedTemp.toFixed(1)}\xB0C), trend=${trendType}`);
          }
        } catch (tempError) {
  ...
  garminWebhook("menstrual-cycle", async (req, res) => {
    try {
      ...
      for (const cycle of cycleData) {
        try {
          const date = cycle.periodStartDate || (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
          let userId;
          for (const device2 of devices) {
            const recentRun = await db.query.runs.findFirst({
              where: and14(
          ...
          const cycleFields = {
            ...
            lastUpdatedAt: new Date(Math.min(cycle.lastUpdatedTimeInSeconds * 1e3, Date.now())),
            rawData: cycle,
            syncedAt: /* @__PURE__ */ new Date()
          };
          if (cycle.pregnancySnapshot) {
            const pregnancy2 = cycle.pregnancySnapshot;
            const pregnancyStartDate = new Date(cycle.pregnancySnapshot.pregnancyCycleStartDate);
            const now = /* @__PURE__ */ new Date();
            const weeksOfPregnancy2 = Math.floor((now.getTime() - pregnancyStartDate.getTime()) / (7 * 24 * 60 * 60 * 1e3));
            Object.assign(cycleFields, {
              pregnancyStatus: "ACTIVE",
              pregnancyTitle: pregnancy2.title,
              originalDueDate: pregnancy2.originalDueDate,
              pregnancyDueDate: pregnancy2.dueDate,
              pregnancyStartDate: pregnancy2.pregnancyCycleStartDate,
              weeksOfPregnancy: weeksOfPregnancy2,
              expectedDeliveryDate: pregnancy2.dueDate,
              deliveryDate: pregnancy2.deliveryDate,
              numberOfBabies: pregnancy2.numOfBabies,
              // SINGLE, TWIN, etc.
              hasExperiencedLoss: pregnancy2.hasExperiencedLoss,
              heightInCentimeters: pregnancy2.weightGoalUserInput?.heightInCentimeters,
              weightInGrams: pregnancy2.weightGoalUserInput?.weightInGrams,
              bloodGlucoseReadings: pregnancy2.bloodGlucoseList || []
            });
          }
          ...
          if (existingCycle && existingCycle.cycleStartDate === cycle.periodStartDate) {
            ...
            if (cycle.pregnancySnapshot) {
              const pregnancy2 = cycle.pregnancySnapshot;
              const weeksOfPregnancy2 = Math.floor((Date.now() - new Date(pregnancy2.pregnancyCycleStartDate).getTime()) / (7 * 24 * 60 * 60 * 1e3));
              console.log(`[Garmin Webhook] Created pregnancy tracking for user ${userId}: ${pregnancy2.title}, ${weeksOfPregnancy2} weeks, due ${pregnancy2.dueDate}`);
            } else {
  ...
  async function calculateWeatherImpact(userId, runs3) {
    if (runs3.length < 3) {
      return { hasEnoughData: false, runsAnalyzed: runs3.length, overallAvgPace: null };
    }
    const parsePace = (paceStr) => {
      if (!paceStr) return null;
      const match = paceStr.match(/(\d+):(\d+)/);
    ...
    const paces = runs3.map((r) => parsePace(r.avgPace)).filter((p) => p !== null);
    if (paces.length === 0) {
      return { hasEnoughData: false, runsAnalyzed: runs3.length, overallAvgPace: null };
    }
    ...
    runs3.forEach((run) => {
      ...
      if (humidity !== void 0 && humidity !== null) {
        let humidityLabel = "";
    ...
    const humidityAnalysis = Object.entries(humidityBuckets).filter(([_, b]) => b.count > 0).map(([label, bucket]) => ({
      range: label,
      label,
      avgPace: Math.round(bucket.totalPace / bucket.count),
      runCount: bucket.count,
  ...
  app2.post("/api/coaching/pre-run-briefing", authMiddleware, async (req, res) => {
    try {
      ...
      try {
        ...
        if (runs3.length >= 3) {
          weatherImpactData = await calculateWeatherImpact(req.user.userId, runs3);
        }
      } catch (err) {
  ...
  app2.post("/api/tts/generate", authMiddleware, async (req, res) => {
    try {
      ...
      const audioBuffer = await aiService.generateTTS(text2, userVoice, talkTTSInstructions, user?.coachAccent, user?.coachGender);
  ...
  const getCoachTTSInstructions = async (coachAccent, coachTone, coachGender, coachName) => {
  ...
  app2.post("/api/coaching/interval-coaching", async (req, res) => {
    try {
      const { coachGender, coachAccent, coachTone: baseTone } = req.body;
      const aiService = await Promise.resolve().then(() => (init_ai_service(), ai_service_exports));
  ...
  app2.post("/api/coaching/talk-to-coach", authMiddleware, async (req, res) => {
    try {
      ...
      context.coachAccent = user?.coachAccent || req.body.coachAccent || void 0;
      const aiService = await Promise.resolve().then(() => (init_ai_service(), ai_service_exports));
  ...
  app2.post("/api/coaching/hr-coaching", authMiddleware, async (req, res) => {
    try {
      ...
      const aiService = await Promise.resolve().then(() => (init_ai_service(), ai_service_exports));
      const runnerAge = req.body.runnerAge ?? user?.age ?? void 0;
  ...
  app2.put("/api/users/:id/coach-settings", authMiddleware, async (req, res) => {
    try {
      ...
      const {
        ...
        coachPaceEnabled,
        coachNavigationEnabled,
        coachElevationEnabled,
        coachHeartRateEnabled,
        coachCadenceStrideEnabled,
        coachKmSplitsEnabled,
        coachStruggleEnabled,
        coachMotivationalEnabled,
        coachHalfKmCheckInEnabled,
      ...
      if (coachKmSplitIntervalKm !== void 0) {
      ...
      const updateData = {};
      if (coachName !== void 0) updateData.coachName = coachName;
      if (coachGender !== void 0) updateData.coachGender = coachGender;
      if (coachAccent !== void 0) updateData.coachAccent = coachAccent;
      if (coachTone !== void 0) updateData.coachTone = coachTone;
      if (coachPaceEnabled !== void 0) updateData.coachPaceEnabled = coachPaceEnabled;
      if (coachNavigationEnabled !== void 0) updateData.coachNavigationEnabled = coachNavigationEnabled;
      if (coachElevationEnabled !== void 0) updateData.coachElevationEnabled = coachElevationEnabled;
      if (coachHeartRateEnabled !== void 0) updateData.coachHeartRateEnabled = coachHeartRateEnabled;
      if (coachCadenceStrideEnabled !== void 0) updateData.coachCadenceStrideEnabled = coachCadenceStrideEnabled;
      if (coachKmSplitsEnabled !== void 0) updateData.coachKmSplitsEnabled = coachKmSplitsEnabled;
      if (coachStruggleEnabled !== void 0) updateData.coachStruggleEnabled = coachStruggleEnabled;
      if (coachMotivationalEnabled !== void 0) updateData.coachMotivationalEnabled = coachMotivationalEnabled;
      if (coachHalfKmCheckInEnabled !== void 0) updateData.coachHalfKmCheckInEnabled = coachHalfKmCheckInEnabled;
      if (coachKmSplitIntervalKm !== void 0) updateData.coachKmSplitIntervalKm = coachKmSplitIntervalKm;
  ...
  app2.get("/api/group-runs/:groupRunId", authMiddleware, async (req, res) => {
    try {
  ...
  app2.post("/api/group-runs/:groupRunId/invite", authMiddleware, async (req, res) => {
    try {
      const { groupRunId } = req.params;
      ...
      for (const uid of userIds) {
        ...
        await db.insert(groupRunParticipants).values({
          ...
          inviteExpiresAt: new Date(Date.now() + 7 * 24 * 60 * 60 * 1e3)
          // 7 days
        });
        results.push(uid);
      }
  ...
  app2.post("/api/group-runs/:groupRunId/respond", authMiddleware, async (req, res) => {
    try {
      const { groupRunId } = req.params;
  ...
  app2.post("/api/group-runs/:groupRunId/ready", authMiddleware, async (req, res) => {
    try {
      const { groupRunId } = req.params;
      const userId = req.user.userId;
  ...
  app2.post("/api/group-runs/:groupRunId/start", authMiddleware, async (req, res) => {
    try {
      const { groupRunId } = req.params;
      const userId = req.user.userId;
      const gr = await storage.getGroupRun(groupRunId);
  ...
  app2.post("/api/group-runs/:groupRunId/complete", authMiddleware, async (req, res) => {
    try {
      const { groupRunId } = req.params;
      const { runId } = req.body;
      const userId = req.user.userId;
  ...
  app2.get("/api/group-runs/:groupRunId/results", authMiddleware, async (req, res) => {
    try {
      const { groupRunId } = req.params;
      const userId = req.user.userId;
      const gr = await storage.getGroupRun(groupRunId);
        ...
        allParticipants.map(async (p) => {
          ...
          return {
            ...
            stats: runData ? {
              ...
              avgHeartRate: runData.avgHeartRate || null,
  ...
  app2.post("/api/coaching/run-analysis", authMiddleware, async (req, res) => {
    try {
        ...
        try {
          ...
          if (recentRuns.length >= 3) {
            ...
            if (weatherImpactData && weatherImpactData.insights) {
              ...
              if (humidity !== void 0 && weatherImpactData.humidityAnalysis) {
      ...
      const allStrugglePoints = Array.isArray(run.strugglePoints) ? run.strugglePoints : [];
      const relevantStrugglePoints = allStrugglePoints.filter(
        (sp) => sp.isRelevant !== false
      );
      const runDataForAi = {
        ...run,
        ...body,
      ...
      try {
        ...
        ai = await aiService.generateComprehensiveRunAnalysis({
          ...
          coachName,
          coachTone,
          coachAccent: user?.coachAccent || body.coachAccent || void 0
        });
      } catch (e) {
      ...
      const trainingRecommendations = (Array.isArray(ai.improvementTips) ? ai.improvementTips : []).slice(0, 6).map((t) => {
        ...
        const category = lower.includes("cadence") || lower.includes("stride") || lower.includes("form") || lower.includes("lean") || lower.includes("foot") || lower.includes("arm") ? "technique" : lower.includes("interval") || lower.includes("tempo") || lower.includes("speed") || lower.includes("threshold") ? "speed" : lower.includes("strength") || lower.includes("hill") || lower.includes("core") || lower.includes("drill") ? "strength" : "endurance";
      ...
      const response = {
        ...
        strugglePointsInsight: String(ai.strugglePointsInsight || (Array.isArray(body.relevantStrugglePoints) && body.relevantStrugglePoints.length ? `We detected ${body.relevantStrugglePoints.length} struggle point(s). Your notes help us interpret them accurately.` : "")) || null,
  ...
  app2.get("/api/segments/:id/leaderboard", authMiddleware, async (req, res) => {
    try {
      ...
      res.json({
        segmentId: id,
        timeframe,
        leaderboard: efforts.map((e, index2) => ({
          rank: index2 + 1,
          userId: e.user?.id,
  ...
  app2.post("/api/training-plans/generate", authMiddleware, async (req, res) => {
    try {
      ...
      const {
        ...
        targetDate,
        durationWeeks,
        // user-selected duration (takes priority over targetDate)
        experienceLevel,
        daysPerWeek,
        firstSessionStart = "flexible",
        // "today" | "tomorrow" | "flexible"
        regularSessions = [],
        // recurring runs the user already does each week
        injuries = [],
        // [{ bodyPart, status, notes }]
        goalId = null,
        // optional: goal ID to link this plan to
        userTimezone = null,
        // IANA timezone name, e.g. "Pacific/Auckland"
  ...
  app2.get("/api/training-plans/details/:planId", authMiddleware, async (req, res) => {
    try {
      ...
      if (recentRuns.length === 0) {
        ...
        const paceValues = recentRuns.map((r) => {
  ...
  app2.put("/api/training-plans/:planId/regenerate", authMiddleware, async (req, res) => {
    try {
      const { planId } = req.params;
      const userId = req.user.userId;
      ...
      const weeksSincePlanStart = Math.floor((today.getTime() - planWeekStart.getTime()) / (7 * 24 * 60 * 60 * 1e3));
      ...
      const validInjuries = (Array.isArray(injuries) ? injuries : []).filter(
        (i) => i && typeof i.bodyPart === "string" && typeof i.status === "string"
      );
      const injuryContext = validInjuries.length > 0 ? `
INJURIES & LIMITATIONS (user updated mid-plan):
${validInjuries.map((i) => `- ${i.bodyPart}: ${i.status}${i.notes ? ` \u2014 ${i.notes}` : ""}`).join("\n")}
...
Include ${plan[0].daysPerWeek} workouts per week.`;
  ...
  app2.get("/api/training-plans/:planId/today", authMiddleware, async (req, res) => {
    try {
      const { planId } = req.params;
      const { timezone } = req.query;
      const today = timezone ? /* @__PURE__ */ new Date((/* @__PURE__ */ new Date()).toLocaleDateString("en-CA", { timeZone: timezone }) + "T00:00:00") : (() => {
  ...
  app2.put("/api/training-plans/workouts/:workoutId/complete", authMiddleware, async (req, res) => {
    try {
      const { workoutId } = req.params;
      const { runId } = req.body;
  ...
  app2.put("/api/training-plans/:planId/status", authMiddleware, async (req, res) => {
    try {
      const { planId } = req.params;
  ...
  app2.delete("/api/training-plans/:planId", authMiddleware, async (req, res) => {
    try {
      const { planId } = req.params;
      const userId = req.user?.userId;
      if (!userId) {
  ...
  app2.post("/api/runs/:id/share-link", authMiddleware, async (req, res) => {
    try {
      const runId = req.params.id;
      const run = await storage.getRun(runId);
  ...
  function buildShareRunData(run) {
    ...
    const gpsTrack = rawGps.length > 1 ? rawGps.map((p) => ({
    ...
    })).filter((p) => p.lat !== 0 && p.lng !== 0) : void 0;
    const rawKmSplits = Array.isArray(run.kmSplits) ? run.kmSplits : [];
    let paceData = run.paceData;
    if ((!paceData || !Array.isArray(paceData) || paceData.length === 0) && rawKmSplits.length > 0) {
      paceData = rawKmSplits.map((s) => {
  ...
  app2.get("/api/health/today", authMiddleware, async (req, res) => {
    try {
      const { getTodaySnapshot: getTodaySnapshot2 } = await Promise.resolve().then(() => (init_health_insights_service(), health_insights_service_exports));
      const snapshot = await getTodaySnapshot2(req.user.userId);
      res.json(snapshot);
    } catch (error) {
  ...
  app2.get("/api/health/sleep", authMiddleware, async (req, res) => {
    try {
      const { getSleepDetails: getSleepDetails2 } = await Promise.resolve().then(() => (init_health_insights_service(), health_insights_service_exports));
      const date = req.query.date || (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
      const details = await getSleepDetails2(req.user.userId, date);
      if (!details) {
  ...
  app2.get("/api/health/recovery", authMiddleware, async (req, res) => {
    try {
      const { getRecoveryStatus: getRecoveryStatus2 } = await Promise.resolve().then(() => (init_health_insights_service(), health_insights_service_exports));
      const status = await getRecoveryStatus2(req.user.userId);
      res.json(status);
    } catch (error) {
  ...
  app2.get("/api/health/daily", authMiddleware, async (req, res) => {
    try {
      const { getDailyWellness: getDailyWellness2 } = await Promise.resolve().then(() => (init_health_insights_service(), health_insights_service_exports));
      const date = req.query.date || (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
      const wellness = await getDailyWellness2(req.user.userId, date);
      if (!wellness) {
  ...
  app2.get("/api/health/metrics", authMiddleware, async (req, res) => {
    try {
      const { getHealthMetrics: getHealthMetrics2 } = await Promise.resolve().then(() => (init_health_insights_service(), health_insights_service_exports));
      const metrics = await getHealthMetrics2(req.user.userId);
      res.json(metrics);
    } catch (error) {
  ...
  app2.get("/api/health/insights", authMiddleware, async (req, res) => {
    try {
      const { getHealthInsights: getHealthInsights2 } = await Promise.resolve().then(() => (init_health_insights_service(), health_insights_service_exports));
      const insights = await getHealthInsights2(req.user.userId);
      res.json(insights);
    } catch (error) {
  ...
  app2.get("/api/health/trends", authMiddleware, async (req, res) => {
    try {
      const { getTrendData: getTrendData2 } = await Promise.resolve().then(() => (init_health_insights_service(), health_insights_service_exports));
      const days = parseInt(req.query.days || "7", 10);
      const trends = await getTrendData2(req.user.userId, days);
      res.json(trends);
    } catch (error) {
...
function formatPaceLanding(pace) {
  if (!pace) return "--:--/km";
  return pace.includes("/km") ? pace : `${pace}/km`;
}
function formatDurationLanding(seconds) {
  const h = Math.floor(seconds / 3600);
  const m = Math.floor(seconds % 3600 / 60);
...
function buildShareLandingPage(share) {
...
function buildShareNotFoundPage() {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
async function syncGarminForUser(device2) {
  ...
  try {
    if (!device2.accessToken) {
      result.error = "No access token";
      return result;
    }
    const today = /* @__PURE__ */ new Date();
    let dataPoints = 0;
    const wellness = await getGarminComprehensiveWellness(device2.accessToken, today);
    if (wellness) {
      const wellnessData = {
        userId: device2.userId,
        date: wellness.date,
      ...
      const existingWellness = await storage.getGarminWellnessByDate(device2.userId, today);
      if (existingWellness) {
    ...
    console.log(`[Scheduler] Synced Garmin data for user ${device2.userId}`);
  } catch (error) {
    result.error = error.message;
    console.error(`[Scheduler] Failed to sync Garmin for user ${device2.userId}:`, error.message);
  }
  return result;
}
async function runGarminSync() {
  ...
  try {
    ...
    const results = await Promise.allSettled(
      allDevices.map((device2) => syncGarminForUser(device2))
    );
...
async function sendCoachingPlanReminders() {
  console.log(`[Scheduler] Checking for coaching plan reminders at ${(/* @__PURE__ */ new Date()).toISOString()}`);
  try {
        ...
        for (const plan of activePlans) {
          ...
          if (todaysWorkouts.length > 0) {
            ...
            await sendCoachingPlanReminder(
              userId,
              workoutName,
              workout.distance || void 0,
              workout.intensity || void 0
            );
            coachingPlanRemindersSent.set(userId, /* @__PURE__ */ new Date());
            sentCount++;
            sentForUser = true;
...
function setupBodyParsing(app2) {
  app2.use("/api/garmin/webhook", express2.json({ limit: "100mb" }));
  app2.use("/api/garmin/webhooks", express2.json({ limit: "100mb" }));
  app2.use(
    express2.json({
      limit: "10mb",
      verify: (req, _res, buf) => {
...
function configureExpoAndLanding(app2) {
  ...
  app2.use("/logos", express2.static(path2.resolve(process.cwd(), "attached_assets/generated_images")));
  app2.use(express2.static(path2.resolve(process.cwd(), "static-build")));
  const webDistPath = path2.resolve(process.cwd(), "dist", "public");
  ...
  app2.use((req, res, next) => {
    ...
    if (isBrowserRequest(req)) {
      const webIndexPath = path2.resolve(webDistPath, "index.html");
...
(async () => {
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    const webDistPath = path2.resolve(process.cwd(), "dist", "public");
    const expoDistPath = path2.resolve(process.cwd(), "dist");
    const landingTemplatePath = path2.resolve(process.cwd(), "server", "templates", "landing-page.html");
    ...
    const appName = getAppName();
    app.use("/assets", express2.static(path2.resolve(process.cwd(), "assets")));
    app.use("/logos", express2.static(path2.resolve(process.cwd(), "attached_assets/generated_images")));
    app.use(express2.static(path2.resolve(process.cwd(), "static-build")));
    if (fs.existsSync(webDistPath)) {
    ...
    app.get("*", (req, res) => {
      ...
      const expoIndexPath = path2.resolve(expoDistPath, "index.html");
